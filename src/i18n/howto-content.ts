...
export const howtoContent: Record<string, HowToContent> = {
  en: {
    ...
    guides: [
      ...
      },
      {
        ...
        intro: 'Upload your own .ttf file and use it in Text and Multileader labels — the font stays saved in your browser for future drawings.',
        steps: [
          'Type <code>FontManager</code> in the terminal to open the dialog directly — or, if you already have the text editor open, click the <strong>Font Manager</strong> button in its toolbar.',
          'Click <strong>Add Font</strong> in the dialog footer and choose a <code>.ttf</code> file — only TrueType fonts are supported.',
          'The file name becomes the font\'s name in the <strong>User</strong> group (uploading <code>MyFont.ttf</code> adds a font named <code>MyFont</code>).',
          'To apply the font, place a new text label (<code>text</code> command) or double-click an existing Text or Multileader to open the text editor.',
          'Select the new font from the list to apply it — with a text selection it overrides just those characters, with no selection it sets the whole label.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Text editor', slug: 'interface/text-editor' },
          { label: 'Text command', slug: 'commands/text' },
        ],
      },
      {
        title: 'Snap and align precisely',
        intro: 'Combine Grid & Snap, Vector Pins, and the Align command to position new geometry exactly — without typing coordinates by hand.',
        steps: [
          'Turn on <strong>Grid</strong> and <strong>Snap</strong> in the control bar to lock the cursor to a regular grid — the spacing adapts as you zoom, and existing geometry snaps (endpoint, midpoint, intersection) still take priority over the grid.',
          'Hover any snap point for half a second to pin it with <strong>Vector Pins</strong> (on by default) — the pin projects dashed horizontal and vertical reference lines so you can click a point that shares its exact X or Y coordinate, without drawing a construction line.',
          'Pin two different points to snap to their intersection — this places a point at exactly (X of one pin, Y of the other), and works together with angle tracking too.',
          'To reposition an existing shape onto reference points instead of drawing new geometry, select it and run <code>align</code> — click a source point then a destination point to move it, or add a second pair to also rotate (and optionally scale) it into place.',
          'Press <strong>Escape</strong> at any time to clear pins or cancel the alignment and start over.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
  ...
  de: {
    ...
    guides: [
      ...
      {
        title: 'Eigene Schriftart hinzufügen',
        intro: 'Laden Sie Ihre eigene .ttf-Datei hoch und verwenden Sie sie in Text- und Multileader-Beschriftungen — die Schriftart bleibt in Ihrem Browser für zukünftige Zeichnungen gespeichert.',
        steps: [
          'Geben Sie <code>FontManager</code> im Terminal ein, um den Dialog direkt zu öffnen — oder klicken Sie, falls der Texteditor bereits geöffnet ist, auf die Schaltfläche <strong>Font Manager</strong> in dessen Symbolleiste.',
          'Klicken Sie im Fußbereich des Dialogs auf <strong>Add Font</strong> und wählen Sie eine <code>.ttf</code>-Datei — nur TrueType-Schriftarten werden unterstützt.',
          'Der Dateiname wird zum Namen der Schriftart in der Gruppe <strong>User</strong> (das Hochladen von <code>MyFont.ttf</code> fügt eine Schriftart namens <code>MyFont</code> hinzu).',
          'Um die Schriftart anzuwenden, platzieren Sie eine neue Textbeschriftung (Befehl <code>text</code>) oder doppelklicken Sie auf einen vorhandenen Text oder Multileader, um den Texteditor zu öffnen.',
          'Wählen Sie die neue Schriftart aus der Liste aus, um sie anzuwenden — bei einer Textauswahl überschreibt sie nur diese Zeichen, ohne Auswahl legt sie die gesamte Beschriftung fest.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Texteditor', slug: 'interface/text-editor' },
          { label: 'Text-Befehl', slug: 'commands/text' },
        ],
      },
      {
        title: 'Präzise einrasten und ausrichten',
        intro: 'Kombinieren Sie Raster & Einrasten, Vector Pins und den Align-Befehl, um neue Geometrie exakt zu positionieren — ganz ohne Koordinaten von Hand einzutippen.',
        steps: [
          'Aktivieren Sie <strong>Grid</strong> und <strong>Snap</strong> in der Bedienleiste, um den Cursor an einem regelmäßigen Raster einrasten zu lassen — der Abstand passt sich beim Zoomen an, und vorhandene Geometrie-Fangpunkte (Endpunkt, Mittelpunkt, Schnittpunkt) haben weiterhin Vorrang vor dem Raster.',
          'Bewegen Sie den Mauszeiger eine halbe Sekunde über einen Fangpunkt, um ihn mit <strong>Vector Pins</strong> zu fixieren (standardmäßig aktiv) — der Pin projiziert gestrichelte horizontale und vertikale Referenzlinien, sodass Sie einen Punkt mit exakt derselben X- oder Y-Koordinate anklicken können, ohne eine Hilfslinie zu zeichnen.',
          'Fixieren Sie zwei verschiedene Punkte, um an ihrem Schnittpunkt einzurasten — so platzieren Sie einen Punkt bei genau (X des einen Pins, Y des anderen), auch in Kombination mit der Winkelverfolgung.',
          'Um eine vorhandene Form auf Referenzpunkte statt neu zu zeichnen, wählen Sie sie aus und führen Sie <code>align</code> aus — klicken Sie einen Quellpunkt und dann einen Zielpunkt, um sie zu verschieben, oder fügen Sie ein zweites Punktpaar hinzu, um sie zusätzlich zu drehen (und optional zu skalieren).',
          'Drücken Sie jederzeit <strong>Escape</strong>, um Pins zu löschen oder die Ausrichtung abzubrechen und neu zu beginnen.',
        ],
        links: [
          { label: 'Raster & Einrasten', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align', slug: 'commands/align' },
        ],
      },
  ...
  es: {
    ...
    guides: [
      ...
      {
        title: 'Añadir una fuente personalizada',
        intro: 'Sube tu propio archivo .ttf y úsalo en las etiquetas de Text y Multileader — la fuente permanece guardada en tu navegador para futuros dibujos.',
        steps: [
          'Escribe <code>FontManager</code> en el terminal para abrir el diálogo directamente — o, si ya tienes el editor de texto abierto, haz clic en el botón <strong>Font Manager</strong> de su barra de herramientas.',
          'Haz clic en <strong>Add Font</strong> en la parte inferior del diálogo y elige un archivo <code>.ttf</code> — solo se admiten fuentes TrueType.',
          'El nombre del archivo se convierte en el nombre de la fuente dentro del grupo <strong>User</strong> (subir <code>MyFont.ttf</code> añade una fuente llamada <code>MyFont</code>).',
          'Para aplicar la fuente, coloca una nueva etiqueta de texto (comando <code>text</code>) o haz doble clic en un Text o Multileader existente para abrir el editor de texto.',
          'Selecciona la nueva fuente de la lista para aplicarla — con una selección de texto solo sobrescribe esos caracteres, sin selección establece toda la etiqueta.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Editor de texto', slug: 'interface/text-editor' },
          { label: 'Comando Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Ajustar y alinear con precisión',
        intro: 'Combina Cuadrícula y Ajuste, Vector Pins y el comando Align para posicionar geometría nueva con exactitud — sin escribir coordenadas a mano.',
        steps: [
          'Activa <strong>Grid</strong> y <strong>Snap</strong> en la barra de control para bloquear el cursor a una cuadrícula regular — el espaciado se adapta al hacer zoom, y los ajustes a geometría existente (extremo, punto medio, intersección) siguen teniendo prioridad sobre la cuadrícula.',
          'Pasa el cursor sobre cualquier punto de ajuste durante medio segundo para fijarlo con <strong>Vector Pins</strong> (activado por defecto) — el pin proyecta líneas de referencia horizontales y verticales discontinuas para que puedas hacer clic en un punto que comparta su coordenada X o Y exacta, sin dibujar una línea de construcción.',
          'Fija dos puntos distintos para ajustarte a su intersección — esto coloca un punto exactamente en (X de un pin, Y del otro), y también funciona junto con el seguimiento de ángulo.',
          'Para reposicionar una forma existente sobre puntos de referencia en lugar de dibujar geometría nueva, selecciónala y ejecuta <code>align</code> — haz clic en un punto de origen y luego en uno de destino para moverla, o añade un segundo par de puntos para además rotarla (y opcionalmente escalarla) hasta su posición.',
          'Pulsa <strong>Escape</strong> en cualquier momento para borrar los pines o cancelar la alineación y empezar de nuevo.',
        ],
        links: [
          { label: 'Cuadrícula y Ajuste', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align', slug: 'commands/align' },
        ],
      },
  ...
  fr: {
    ...
    guides: [
      ...
      {
        title: 'Ajouter une police personnalisée',
        intro: 'Téléversez votre propre fichier .ttf et utilisez-le dans les labels Text et Multileader — la police reste enregistrée dans votre navigateur pour vos futurs dessins.',
        steps: [
          'Tapez <code>FontManager</code> dans le terminal pour ouvrir directement le dialogue — ou, si l\'éditeur de texte est déjà ouvert, cliquez sur le bouton <strong>Font Manager</strong> dans sa barre d\'outils.',
          'Cliquez sur <strong>Add Font</strong> en bas du dialogue et choisissez un fichier <code>.ttf</code> — seules les polices TrueType sont prises en charge.',
          'Le nom du fichier devient le nom de la police dans le groupe <strong>User</strong> (téléverser <code>MyFont.ttf</code> ajoute une police nommée <code>MyFont</code>).',
          'Pour appliquer la police, placez un nouveau label de texte (commande <code>text</code>) ou double-cliquez sur un Text ou Multileader existant pour ouvrir l\'éditeur de texte.',
          'Sélectionnez la nouvelle police dans la liste pour l\'appliquer — avec une sélection de texte, elle ne remplace que ces caractères ; sans sélection, elle définit tout le label.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Éditeur de texte', slug: 'interface/text-editor' },
          { label: 'Commande Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Accrocher et aligner avec précision',
        intro: 'Combinez Grille et Accrochage, Vector Pins et la commande Align pour positionner une nouvelle géométrie avec exactitude — sans saisir de coordonnées à la main.',
        steps: [
          'Activez <strong>Grid</strong> et <strong>Snap</strong> dans la barre de contrôle pour verrouiller le curseur sur une grille régulière — l\'espacement s\'adapte au zoom, et les accrochages sur la géométrie existante (extrémité, milieu, intersection) restent toujours prioritaires sur la grille.',
          'Survolez un point d\'accrochage pendant une demi-seconde pour l\'épingler avec <strong>Vector Pins</strong> (activé par défaut) — l\'épingle projette des lignes de référence horizontales et verticales en pointillés, ce qui permet de cliquer sur un point partageant exactement sa coordonnée X ou Y, sans tracer de ligne de construction.',
          'Épinglez deux points différents pour accrocher leur intersection — cela place un point exactement à (X d\'une épingle, Y de l\'autre), et fonctionne aussi avec le suivi d\'angle.',
          'Pour repositionner une forme existante sur des points de référence plutôt que de dessiner une nouvelle géométrie, sélectionnez-la et exécutez <code>align</code> — cliquez un point source puis un point de destination pour la déplacer, ou ajoutez une deuxième paire pour aussi la faire pivoter (et éventuellement la mettre à l\'échelle).',
          'Appuyez sur <strong>Escape</strong> à tout moment pour effacer les épingles ou annuler l\'alignement et recommencer.',
        ],
        links: [
          { label: 'Grille et Accrochage', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  it: {
    ...
    guides: [
      ...
      {
        title: 'Aggiungere un font personalizzato',
        intro: 'Carica il tuo file .ttf e usalo nelle etichette Text e Multileader — il font rimane salvato nel tuo browser per i disegni futuri.',
        steps: [
          'Digita <code>FontManager</code> nel terminale per aprire direttamente la finestra di dialogo — oppure, se hai già l\'editor di testo aperto, clicca il pulsante <strong>Font Manager</strong> nella sua barra degli strumenti.',
          'Clicca <strong>Add Font</strong> in fondo alla finestra di dialogo e scegli un file <code>.ttf</code> — sono supportati solo i font TrueType.',
          'Il nome del file diventa il nome del font nel gruppo <strong>User</strong> (caricare <code>MyFont.ttf</code> aggiunge un font chiamato <code>MyFont</code>).',
          'Per applicare il font, posiziona una nuova etichetta di testo (comando <code>text</code>) o fai doppio clic su un Text o Multileader esistente per aprire l\'editor di testo.',
          'Seleziona il nuovo font dall\'elenco per applicarlo — con una selezione di testo sostituisce solo quei caratteri, senza selezione imposta l\'intera etichetta.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Editor di testo', slug: 'interface/text-editor' },
      ...
      {
        title: 'Agganciare e allineare con precisione',
        intro: 'Combina Griglia e Snap, Vector Pins e il comando Align per posizionare la nuova geometria con esattezza — senza digitare le coordinate a mano.',
        steps: [
          'Attiva <strong>Grid</strong> e <strong>Snap</strong> nella barra di controllo per bloccare il cursore su una griglia regolare — la spaziatura si adatta allo zoom, e gli aggangi alla geometria esistente (estremità, punto medio, intersezione) hanno comunque priorità sulla griglia.',
          'Passa il cursore su un punto di aggancio per mezzo secondo per fissarlo con <strong>Vector Pins</strong> (attivo per impostazione predefinita) — il pin proietta linee di riferimento tratteggiate orizzontali e verticali, così puoi cliccare un punto che condivide esattamente la sua coordinata X o Y, senza disegnare una linea di costruzione.',
          'Fissa due punti diversi per agganciarti alla loro intersezione — questo posiziona un punto esattamente a (X di un pin, Y dell\'altro), e funziona anche insieme al tracciamento dell\'angolo.',
          'Per riposizionare una forma esistente su punti di riferimento invece di disegnare nuova geometria, selezionala ed esegui <code>align</code> — clicca un punto sorgente e poi uno di destinazione per spostarla, oppure aggiungi una seconda coppia per ruotarla (e facoltativamente ridimensionarla) in posizione.',
          'Premi <strong>Escape</strong> in qualsiasi momento per cancellare i pin o annullare l\'allineamento e ricominciare.',
        ],
        links: [
          { label: 'Griglia e Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  pt: {
    ...
    guides: [
      ...
      {
        title: 'Adicionar uma fonte personalizada',
        intro: 'Envie seu próprio arquivo .ttf e use-o em etiquetas Text e Multileader — a fonte permanece salva no seu navegador para desenhos futuros.',
        steps: [
          'Digite <code>FontManager</code> no terminal para abrir a caixa de diálogo diretamente — ou, se você já tiver o editor de texto aberto, clique no botão <strong>Font Manager</strong> em sua barra de ferramentas.',
          'Clique em <strong>Add Font</strong> no rodapé da caixa de diálogo e escolha um arquivo <code>.ttf</code> — apenas fontes TrueType são suportadas.',
          'O nome do arquivo se torna o nome da fonte no grupo <strong>User</strong> (enviar <code>MyFont.ttf</code> adiciona uma fonte chamada <code>MyFont</code>).',
          'Para aplicar a fonte, coloque uma nova etiqueta de texto (comando <code>text</code>) ou clique duas vezes em um Text ou Multileader existente para abrir o editor de texto.',
          'Selecione a nova fonte na lista para aplicá-la — com uma seleção de texto ela substitui apenas esses caracteres, sem seleção ela define toda a etiqueta.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Editor de texto', slug: 'interface/text-editor' },
      ...
      {
        title: 'Ajustar e alinhar com precisão',
        intro: 'Combine Grade e Snap, Vector Pins e o comando Align para posicionar geometria nova com exatidão — sem digitar coordenadas manualmente.',
        steps: [
          'Ative <strong>Grid</strong> e <strong>Snap</strong> na barra de controle para travar o cursor a uma grade regular — o espaçamento se adapta ao zoom, e os ajustes à geometria existente (extremidade, ponto médio, interseção) continuam tendo prioridade sobre a grade.',
          'Passe o cursor sobre um ponto de ajuste por meio segundo para fixá-lo com <strong>Vector Pins</strong> (ativado por padrão) — o pin projeta linhas de referência horizontais e verticais tracejadas, para que você clique em um ponto que compartilhe exatamente sua coordenada X ou Y, sem desenhar uma linha de construção.',
          'Fixe dois pontos diferentes para ajustar à interseção deles — isso posiciona um ponto exatamente em (X de um pin, Y do outro), e também funciona junto com o rastreamento de ângulo.',
          'Para reposicionar uma forma existente sobre pontos de referência em vez de desenhar geometria nova, selecione-a e execute <code>align</code> — clique em um ponto de origem e depois em um de destino para movê-la, ou adicione um segundo par para também rotacioná-la (e opcionalmente redimensioná-la) na posição.',
          'Pressione <strong>Escape</strong> a qualquer momento para limpar os pins ou cancelar o alinhamento e recomeçar.',
        ],
        links: [
          { label: 'Grade e Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  uk: {
    ...
    guides: [
      ...
      {
        title: 'Додати власний шрифт',
        intro: 'Завантажте власний файл .ttf і використовуйте його в мітках Text та Multileader — шрифт залишається збереженим у вашому браузері для майбутніх креслеників.',
        steps: [
          'Введіть <code>FontManager</code> у терміналі, щоб відкрити діалог напряму — або, якщо текстовий редактор вже відкрито, натисніть кнопку <strong>Font Manager</strong> на його панелі інструментів.',
          'Натисніть <strong>Add Font</strong> у нижній частині діалогу та виберіть файл <code>.ttf</code> — підтримуються лише шрифти TrueType.',
          'Назва файлу стає назвою шрифту в групі <strong>User</strong> (завантаження <code>MyFont.ttf</code> додає шрифт з назвою <code>MyFont</code>).',
          'Щоб застосувати шрифт, розмістіть нову текстову мітку (команда <code>text</code>) або двічі клацніть на наявному Text чи Multileader, щоб відкрити текстовий редактор.',
          'Виберіть новий шрифт зі списку, щоб застосувати його — з виділеним текстом він змінює лише ці символи, без виділення він встановлюється для всієї мітки.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Текстовий редактор', slug: 'interface/text-editor' },
          { label: 'Команда Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Точна прив\'язка та вирівнювання',
        intro: 'Поєднуйте Сітку та прив\'язку, Vector Pins і команду Align, щоб точно розташовувати нову геометрію — без ручного введення координат.',
        steps: [
          'Увімкніть <strong>Grid</strong> і <strong>Snap</strong> на панелі керування, щоб прив\'язати курсор до регулярної сітки — крок адаптується під час масштабування, а прив\'язки до наявної геометрії (кінцева точка, середина, перетин) все одно мають пріоритет над сіткою.',
          'Затримайте курсор на будь-якій точці прив\'язки на півсекунди, щоб закріпити її за допомогою <strong>Vector Pins</strong> (увімкнено за замовчуванням) — шпилька проєктує пунктирні горизонтальну та вертикальну лінії прив\'язки, щоб ви могли клацнути точку з тією ж X або Y координатою, не малюючи допоміжну лінію.',
          'Закріпіть дві різні точки, щоб прив\'язатися до їхнього перетину — це розміщує точку точно в (X однієї шпильки, Y іншої), і працює також разом з відстеженням кута.',
          'Щоб перемістити наявну фігуру на опорні точки замість малювання нової геометрії, виберіть її та виконайте <code>align</code> — клацніть вихідну точку, а потім точку призначення, щоб перемістити її, або додайте другу пару, щоб також повернути (і за бажанням масштабувати) її на місце.',
          'Натисніть <strong>Escape</strong> у будь-який момент, щоб очистити шпильки або скасувати вирівнювання й почати заново.',
        ],
        links: [
          { label: 'Сітка та прив\'язка', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  tr: {
    ...
    guides: [
      ...
      {
        title: 'Özel yazı tipi ekleme',
        intro: 'Kendi .ttf dosyanızı yükleyin ve Text ile Multileader etiketlerinde kullanın — yazı tipi, gelecekteki çizimleriniz için tarayıcınızda kayıtlı kalır.',
        steps: [
          'İletişim kutusunu doğrudan açmak için terminale <code>FontManager</code> yazın — veya metin düzenleyici zaten açıksa, araç çubuğundaki <strong>Font Manager</strong> düğmesine tıklayın.',
          'İletişim kutusunun altındaki <strong>Add Font</strong>\'a tıklayın ve bir <code>.ttf</code> dosyası seçin — yalnızca TrueType yazı tipleri desteklenir.',
          'Dosya adı <strong>User</strong> grubunda yazı tipinin adı olur (<code>MyFont.ttf</code> yüklemek <code>MyFont</code> adlı bir yazı tipi ekler).',
          'Yazı tipini uygulamak için yeni bir metin etiketi yerleştirin (<code>text</code> komutu) veya metin düzenleyiciyi açmak için mevcut bir Text ya da Multileader\'a çift tıklayın.',
          'Uygulamak için listeden yeni yazı tipini seçin — bir metin seçiliyken yalnızca o karakterleri değiştirir, seçim yokken tüm etiketi ayarlar.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Metin düzenleyici', slug: 'interface/text-editor' },
          { label: 'Text komutu', slug: 'commands/text' },
        ],
      },
      {
        title: 'Hassas yakalama ve hizalama',
        intro: 'Yeni geometriyi elle koordinat girmeden tam olarak konumlandırmak için Izgara ve Yakalama, Vector Pins ve Align komutunu birleştirin.',
        steps: [
          'İmleci düzenli bir ızgaraya kilitlemek için kontrol çubuğunda <strong>Grid</strong> ve <strong>Snap</strong>\'i açın — aralık yakınlaştırdıkça uyum sağlar ve mevcut geometri yakalamaları (uç nokta, orta nokta, kesişim) yine ızgaraya göre önceliklidir.',
          '<strong>Vector Pins</strong> ile sabitlemek için herhangi bir yakalama noktasının üzerinde yarım saniye bekletin (varsayılan olarak açık) — pin, kesikli yatay ve dikey referans çizgileri yansıtır, böylece bir yapı çizgisi çizmeden tam olarak aynı X veya Y koordinatını paylaşan bir noktaya tıklayabilirsiniz.',
          'Kesişimlerine yakalamak için iki farklı noktayı sabitleyin — bu, bir noktayı tam olarak (bir pinin X\'i, diğerinin Y\'si) konumuna yerleştirir ve açı takibiyle birlikte de çalışır.',
          'Yeni geometri çizmek yerine mevcut bir şekli referans noktalarına yeniden konumlandırmak için onu seçin ve <code>align</code> komutunu çalıştırın — taşımak için bir kaynak nokta, ardından bir hedef nokta tıklayın veya yerine döndürmek (ve isteğe bağlı olarak ölçeklendirmek) için ikinci bir çift ekleyin.',
          'Pinleri temizlemek veya hizalamayı iptal edip yeniden başlamak için istediğiniz zaman <strong>Escape</strong> tuşuna basın.',
        ],
        links: [
          { label: 'Izgara ve Yakalama', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  zh: {
    ...
    guides: [
      ...
      {
        title: '添加自定义字体',
        intro: '上传您自己的 .ttf 文件，并在 Text 和 Multileader 标签中使用它 — 字体会保存在您的浏览器中，供以后的图纸使用。',
        steps: [
          '在命令行中输入 <code>FontManager</code> 直接打开对话框 — 或者，如果文字编辑器已经打开，单击其工具栏中的 <strong>Font Manager</strong> 按钮。',
          '单击对话框底部的 <strong>Add Font</strong>，然后选择一个 <code>.ttf</code> 文件 — 仅支持 TrueType 字体。',
          '文件名将成为 <strong>User</strong> 分组中该字体的名称（上传 <code>MyFont.ttf</code> 会添加一个名为 <code>MyFont</code> 的字体）。',
          '要应用该字体，放置一个新的文字标签（<code>text</code> 命令），或双击现有的 Text 或 Multileader 以打开文字编辑器。',
          '从列表中选择新字体以应用它 — 如果有文字选中，则仅覆盖这些字符；如果没有选中，则设置整个标签。',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: '文字编辑器', slug: 'interface/text-editor' },
          { label: 'Text 命令', slug: 'commands/text' },
        ],
      },
      {
        title: '精确捕捉与对齐',
        intro: '结合使用栅格与捕捉、Vector Pins 和 Align 命令，无需手动输入坐标即可精确定位新几何图形。',
        steps: [
          '在控制栏中打开 <strong>Grid</strong> 和 <strong>Snap</strong>，将光标锁定到规则栅格上 — 间距会随缩放自动调整，现有几何图形的捕捉（端点、中点、交点）仍优先于栅格。',
          '将光标悬停在任意捕捉点上半秒钟，即可用 <strong>Vector Pins</strong>（默认开启）固定该点 — 图钉会投射出虚线水平和垂直参考线，让你无需绘制辅助线即可点击与其 X 或 Y 坐标完全相同的点。',
          '固定两个不同的点以捕捉它们的交点 — 这会将点精确放置在（一个图钉的 X，另一个图钉的 Y）处，并且也能与角度追踪配合使用。',
          '若要将现有图形重新定位到参考点，而不是绘制新几何图形，请选中它并运行 <code>align</code> — 单击一个源点再单击一个目标点即可移动它，或添加第二对点以同时旋转（并可选缩放）到位。',
          '随时按 <strong>Escape</strong> 清除图钉或取消对齐操作重新开始。',
        ],
        links: [
          { label: '栅格与捕捉', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align', slug: 'commands/align' },
        ],
      },
  ...
  hi: {
    ...
    guides: [
      ...
      {
        title: 'कस्टम फ़ॉन्ट जोड़ें',
        intro: 'अपनी खुद की .ttf फ़ाइल अपलोड करें और इसे Text और Multileader लेबल में उपयोग करें — फ़ॉन्ट भविष्य की ड्रॉइंग के लिए आपके ब्राउज़र में सेव रहता है।',
        steps: [
          'डायलॉग सीधे खोलने के लिए टर्मिनल में <code>FontManager</code> टाइप करें — या, अगर टेक्स्ट एडिटर पहले से खुला है, तो उसके टूलबार में <strong>Font Manager</strong> बटन पर क्लिक करें।',
          'डायलॉग के फ़ुटर में <strong>Add Font</strong> पर क्लिक करें और एक <code>.ttf</code> फ़ाइल चुनें — केवल TrueType फ़ॉन्ट समर्थित हैं।',
          'फ़ाइल का नाम <strong>User</strong> समूह में फ़ॉन्ट का नाम बन जाता है (<code>MyFont.ttf</code> अपलोड करने से <code>MyFont</code> नाम का फ़ॉन्ट जुड़ता है)।',
          'फ़ॉन्ट लागू करने के लिए, एक नया टेक्स्ट लेबल रखें (<code>text</code> कमांड) या टेक्स्ट एडिटर खोलने के लिए किसी मौजूदा Text या Multileader पर डबल-क्लिक करें।',
          'इसे लागू करने के लिए सूची से नया फ़ॉन्ट चुनें — टेक्स्ट चयन के साथ यह केवल उन अक्षरों को बदलता है, बिना चयन के यह पूरे लेबल को सेट करता है।',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Text Editor', slug: 'interface/text-editor' },
          { label: 'Text कमांड', slug: 'commands/text' },
        ],
      },
      {
        title: 'सटीक स्नैप और अलाइनमेंट',
        intro: 'नई ज्यामिति को हाथ से कोऑर्डिनेट टाइप किए बिना बिल्कुल सटीक स्थान पर रखने के लिए ग्रिड और स्नैप, Vector Pins, और Align कमांड को साथ में उपयोग करें।',
        steps: [
          'कर्सर को नियमित ग्रिड पर लॉक करने के लिए कंट्रोल बार में <strong>Grid</strong> और <strong>Snap</strong> चालू करें — ज़ूम करने पर स्पेसिंग अपने आप एडजस्ट होती है, और मौजूदा ज्योमेट्री के स्नैप (एंडपॉइंट, मिडपॉइंट, इंटरसेक्शन) अब भी ग्रिड से पहले प्राथमिकता पाते हैं।',
          'किसी भी स्नैप पॉइंट पर आधे सेकंड के लिए कर्सर रोककर उसे <strong>Vector Pins</strong> (डिफ़ॉल्ट रूप से चालू) से पिन करें — पिन डैश्ड हॉरिजॉन्टल और वर्टिकल रेफरेंस लाइनें प्रोजेक्ट करता है, ताकि आप बिना कंस्ट्रक्शन लाइन खींचे उसी X या Y कोऑर्डिनेट वाले पॉइंट पर क्लिक कर सकें।',
          'दो अलग-अलग पॉइंट्स को पिन करके उनके इंटरसेक्शन पर स्नैप करें — इससे पॉइंट ठीक (एक पिन का X, दूसरे का Y) पर रखा जाता है, और यह एंगल ट्रैकिंग के साथ भी काम करता है।',
          'नई ज्यामिति बनाने के बजाय किसी मौजूदा आकृति को रेफरेंस पॉइंट्स पर फिर से स्थापित करने के लिए, उसे चुनें और <code>align</code> चलाएं — इसे मूव करने के लिए एक सोर्स पॉइंट फिर एक डेस्टिनेशन पॉइंट क्लिक करें, या इसे घुमाने (और वैकल्पिक रूप से स्केल करने) के लिए दूसरा पॉइंट पेयर जोड़ें।',
          'पिन साफ़ करने या अलाइनमेंट रद्द करके फिर से शुरू करने के लिए किसी भी समय <strong>Escape</strong> दबाएं।',
        ],
        links: [
          { label: 'ग्रिड और स्नैप', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  ar: {
    ...
    guides: [
      ...
      {
        title: 'إضافة خط مخصص',
        intro: 'ارفع ملف .ttf الخاص بك واستخدمه في تسميات Text وMultileader — يبقى الخط محفوظاً في متصفحك للرسومات المستقبلية.',
        steps: [
          'اكتب <code>FontManager</code> في المحطة لفتح مربع الحوار مباشرة — أو، إذا كان محرر النص مفتوحاً بالفعل، انقر على زر <strong>Font Manager</strong> في شريط أدواته.',
          'انقر على <strong>Add Font</strong> في تذييل الحوار واختر ملف <code>.ttf</code> — تُدعم خطوط TrueType فقط.',
          'يصبح اسم الملف اسم الخط في مجموعة <strong>User</strong> (رفع <code>MyFont.ttf</code> يضيف خطاً باسم <code>MyFont</code>).',
          'لتطبيق الخط، ضع تسمية نص جديدة (أمر <code>text</code>) أو انقر نقراً مزدوجاً على Text أو Multileader موجود لفتح محرر النص.',
          'اختر الخط الجديد من القائمة لتطبيقه — مع تحديد نص فإنه يغيّر تلك الأحرف فقط، وبدون تحديد فإنه يضبط التسمية بأكملها.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'محرر النص', slug: 'interface/text-editor' },
          { label: 'أمر Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'الالتقاط والمحاذاة بدقة',
        intro: 'ادمج بين الشبكة والالتقاط، وVector Pins، وأمر Align لوضع الهندسة الجديدة بدقة تامة — دون كتابة الإحداثيات يدوياً.',
        steps: [
          'فعّل <strong>Grid</strong> و<strong>Snap</strong> في شريط التحكم لتثبيت المؤشر على شبكة منتظمة — تتكيف المسافة أثناء التكبير، وتظل نقاط التقاط الهندسة الموجودة (نقطة النهاية، منتصف، تقاطع) لها الأولوية على الشبكة.',
          'حرّك المؤشر فوق أي نقطة التقاط لمدة نصف ثانية لتثبيتها باستخدام <strong>Vector Pins</strong> (مفعّل افتراضياً) — يعرض الدبوس خطوط مرجعية أفقية وعمودية متقطعة، مما يتيح لك النقر على نقطة تشترك في نفس إحداثي X أو Y تماماً، دون رسم خط إنشاء.',
          'ثبّت نقطتين مختلفتين للالتقاط عند تقاطعهما — هذا يضع نقطة بالضبط عند (X لدبوس واحد، Y للآخر)، ويعمل أيضاً مع تتبع الزاوية.',
          'لإعادة وضع شكل موجود على نقاط مرجعية بدلاً من رسم هندسة جديدة، حدده وشغّل <code>align</code> — انقر نقطة مصدر ثم نقطة وجهة لتحريكه، أو أضف زوجاً ثانياً لتدويره أيضاً (وتحجيمه اختيارياً) في مكانه.',
          'اضغط <strong>Escape</strong> في أي وقت لمسح الدبابيس أو إلغاء المحاذاة والبدء من جديد.',
        ],
        links: [
          { label: 'الشبكة والالتقاط', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  id: {
    ...
    guides: [
      ...
      },
      {
        ...
        intro: 'Unggah file .ttf Anda sendiri dan gunakan di label Text dan Multileader — font tetap tersimpan di browser Anda untuk gambar mendatang.',
        steps: [
          'Ketik <code>FontManager</code> di terminal untuk langsung membuka dialog — atau, jika Anda sudah membuka text editor, klik tombol <strong>Font Manager</strong> di toolbarnya.',
          'Klik <strong>Add Font</strong> di footer dialog dan pilih file <code>.ttf</code> — hanya font TrueType yang didukung.',
          'Nama file menjadi nama font di grup <strong>User</strong> (mengunggah <code>MyFont.ttf</code> menambahkan font bernama <code>MyFont</code>).',
          'Untuk menerapkan font, tempatkan label teks baru (perintah <code>text</code>) atau klik dua kali pada Text atau Multileader yang ada untuk membuka text editor.',
          'Pilih font baru dari daftar untuk menerapkannya — dengan seleksi teks, hanya karakter tersebut yang berubah; tanpa seleksi, seluruh label yang diatur.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Text editor', slug: 'interface/text-editor' },
          { label: 'Perintah Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Snap dan penjajaran presisi',
        intro: 'Gabungkan Grid & Snap, Vector Pins, dan perintah Align untuk memposisikan geometri baru secara presisi — tanpa mengetik koordinat secara manual.',
        steps: [
          'Aktifkan <strong>Grid</strong> dan <strong>Snap</strong> di control bar untuk mengunci kursor ke grid reguler — jaraknya menyesuaikan saat Anda zoom, dan snap ke geometri yang ada (endpoint, midpoint, perpotongan) tetap diprioritaskan di atas grid.',
          'Arahkan kursor ke titik snap mana pun selama setengah detik untuk menyematkannya dengan <strong>Vector Pins</strong> (aktif secara default) — pin memproyeksikan garis referensi horizontal dan vertikal putus-putus, sehingga Anda bisa mengklik titik yang memiliki koordinat X atau Y yang sama persis, tanpa menggambar garis bantu.',
          'Sematkan dua titik berbeda untuk snap ke perpotongannya — ini menempatkan titik tepat di (X dari satu pin, Y dari pin lainnya), dan juga berfungsi bersama pelacakan sudut.',
          'Untuk memposisikan ulang bentuk yang ada ke titik referensi alih-alih menggambar geometri baru, pilih bentuk tersebut dan jalankan <code>align</code> — klik titik sumber lalu titik tujuan untuk memindahkannya, atau tambahkan pasangan titik kedua untuk juga memutar (dan secara opsional mengubah skala) ke posisinya.',
          'Tekan <strong>Escape</strong> kapan saja untuk menghapus pin atau membatalkan penjajaran dan memulai kembali.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  ja: {
    ...
    guides: [
      ...
      {
        title: 'カスタムフォントを追加する',
        intro: '独自の .ttf ファイルをアップロードして、Text や Multileader のラベルで使用できます — フォントはブラウザに保存され、今後の図面でも使えます。',
        steps: [
          'ターミナルに <code>FontManager</code> と入力するとダイアログが直接開きます — すでにテキストエディターを開いている場合は、そのツールバーの <strong>Font Manager</strong> ボタンをクリックしてください。',
          'ダイアログ下部の <strong>Add Font</strong> をクリックし、<code>.ttf</code> ファイルを選択します — 対応しているのは TrueType フォントのみです。',
          'ファイル名が <strong>User</strong> グループでのフォント名になります（<code>MyFont.ttf</code> をアップロードすると <code>MyFont</code> という名前のフォントが追加されます）。',
          'フォントを適用するには、新しいテキストラベルを配置する（<code>text</code> コマンド）か、既存の Text または Multileader をダブルクリックしてテキストエディターを開きます。',
          'リストから新しいフォントを選択して適用します — テキストを選択している場合はその文字のみに適用され、選択していない場合はラベル全体に設定されます。',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'テキストエディター', slug: 'interface/text-editor' },
          { label: 'Text コマンド', slug: 'commands/text' },
        ],
      },
      {
        title: '正確にスナップして整列する',
        intro: 'グリッドとスナップ、Vector Pins、Align コマンドを組み合わせることで、座標を手入力せずに新しいジオメトリを正確に配置できます。',
        steps: [
          'コントロールバーで <strong>Grid</strong> と <strong>Snap</strong> をオンにして、カーソルを規則的なグリッドにロックします — 間隔はズームに応じて自動調整され、既存ジオメトリへのスナップ（端点、中点、交点）は引き続きグリッドより優先されます。',
          '任意のスナップポイントに0.5秒間カーソルを重ねると <strong>Vector Pins</strong>（デフォルトでオン）でピン留めできます — ピンが破線の水平・垂直の参照線を投影するので、補助線を引かなくても同じ X または Y 座標を持つ点をクリックできます。',
          '異なる2点をピン留めすると、その交点にスナップできます — これにより、（一方のピンの X、もう一方の Y）の位置に正確に点が置かれ、角度トラッキングとも併用できます。',
          '新しいジオメトリを描く代わりに既存の図形を参照点に再配置するには、それを選択して <code>align</code> を実行します — ソース点をクリックしてからターゲット点をクリックすると移動し、2組目の点を追加すると回転（および任意で拡大縮小）も同時に行えます。',
          'いつでも <strong>Escape</strong> を押すと、ピンをクリアするか、整列をキャンセルしてやり直せます。',
        ],
        links: [
          { label: 'グリッドとスナップ', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align（整列）', slug: 'commands/align' },
        ],
      },
    ],
  },
  ...
  pl: {
    ...
    guides: [
      ...
      {
        title: 'Dodawanie niestandardowej czcionki',
        intro: 'Prześlij własny plik .ttf i użyj go w etykietach Text i Multileader — czcionka pozostaje zapisana w przeglądarce na potrzeby przyszłych rysunków.',
        steps: [
          'Wpisz <code>FontManager</code> w terminalu, aby otworzyć okno dialogowe bezpośrednio — lub, jeśli edytor tekstu jest już otwarty, kliknij przycisk <strong>Font Manager</strong> na jego pasku narzędzi.',
          'Kliknij <strong>Add Font</strong> w stopce okna dialogowego i wybierz plik <code>.ttf</code> — obsługiwane są tylko czcionki TrueType.',
          'Nazwa pliku staje się nazwą czcionki w grupie <strong>User</strong> (przesłanie <code>MyFont.ttf</code> dodaje czcionkę o nazwie <code>MyFont</code>).',
          'Aby zastosować czcionkę, umieść nową etykietę tekstową (polecenie <code>text</code>) lub kliknij dwukrotnie istniejący Text bądź Multileader, aby otworzyć edytor tekstu.',
          'Wybierz nową czcionkę z listy, aby ją zastosować — przy zaznaczonym tekście zmienia tylko te znaki, bez zaznaczenia ustawia całą etykietę.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Edytor tekstu', slug: 'interface/text-editor' },
          { label: 'Polecenie Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Precyzyjne przyciąganie i wyrównywanie',
        intro: 'Połącz Siatkę i przyciąganie, Vector Pins oraz polecenie Align, aby precyzyjnie pozycjonować nową geometrię — bez ręcznego wpisywania współrzędnych.',
        steps: [
          'Włącz <strong>Grid</strong> i <strong>Snap</strong> na pasku sterowania, aby zablokować kursor do regularnej siatki — odstęp dostosowuje się podczas powiększania, a przyciąganie do istniejącej geometrii (punkt końcowy, środkowy, przecięcie) nadal ma priorytet nad siatką.',
          'Najedź kursorem na dowolny punkt przyciągania na pół sekundy, aby przypiąć go za pomocą <strong>Vector Pins</strong> (domyślnie włączone) — pinezka rzutuje przerywane linie odniesienia poziome i pionowe, dzięki czemu możesz kliknąć punkt o dokładnie tej samej współrzędnej X lub Y, bez rysowania linii pomocniczej.',
          'Przypnij dwa różne punkty, aby przyciągnąć do ich przecięcia — umieszcza to punkt dokładnie w (X jednej pinezki, Y drugiej), i działa również razem ze śledzeniem kąta.',
          'Aby przenieść istniejący kształt na punkty odniesienia zamiast rysować nową geometrię, zaznacz go i uruchom <code>align</code> — kliknij punkt źródłowy, a następnie docelowy, aby go przesunąć, lub dodaj drugą parę, aby także go obrócić (i opcjonalnie przeskalować) na miejsce.',
          'Naciśnij <strong>Escape</strong> w dowolnym momencie, aby wyczyścić pinezki lub anulować wyrównanie i zacząć od nowa.',
        ],
        links: [
          { label: 'Siatka i przyciąganie', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  ko: {
    ...
    guides: [
      ...
      {
        title: '사용자 지정 폰트 추가하기',
        intro: '자신의 .ttf 파일을 업로드하여 Text 및 Multileader 레이블에 사용하세요 — 폰트는 앞으로의 도면을 위해 브라우저에 저장된 상태로 유지됩니다.',
        steps: [
          '터미널에 <code>FontManager</code>를 입력하면 대화 상자가 바로 열립니다 — 이미 텍스트 편집기가 열려 있다면 도구 모음의 <strong>Font Manager</strong> 버튼을 클릭하세요.',
          '대화 상자 하단의 <strong>Add Font</strong>를 클릭하고 <code>.ttf</code> 파일을 선택합니다 — TrueType 폰트만 지원됩니다.',
          '파일 이름이 <strong>User</strong> 그룹에서 폰트 이름이 됩니다(<code>MyFont.ttf</code>를 업로드하면 <code>MyFont</code>라는 이름의 폰트가 추가됩니다).',
          '글꼴을 적용하려면 새 텍스트 레이블을 배치하거나(<code>text</code> 명령어) 기존 Text 또는 Multileader를 더블클릭하여 텍스트 편집기를 엽니다.',
          '목록에서 새 폰트를 선택하여 적용합니다 — 텍스트가 선택된 경우 해당 문자에만 적용되고, 선택하지 않은 경우 레이블 전체에 적용됩니다.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: '텍스트 편집기', slug: 'interface/text-editor' },
          { label: 'Text 명령어', slug: 'commands/text' },
        ],
      },
      {
        title: '정확하게 스냅하고 정렬하기',
        intro: '그리드 & 스냅, Vector Pins, Align 명령어를 함께 사용하면 좌표를 직접 입력하지 않고도 새 도형을 정확하게 배치할 수 있습니다.',
        steps: [
          '커서를 규칙적인 그리드에 고정하려면 컨트롤 바에서 <strong>Grid</strong>와 <strong>Snap</strong>을 켜세요 — 간격은 확대/축소에 따라 자동으로 조정되며, 기존 도형에 대한 스냅(끝점, 중간점, 교차점)은 여전히 그리드보다 우선합니다.',
          '아무 스냅 포인트 위에 0.5초간 커서를 올려두면 <strong>Vector Pins</strong>(기본적으로 켜짐)로 고정됩니다 — 핀이 점선 형태의 수평·수직 참조선을 투영하므로, 보조선을 그리지 않고도 정확히 같은 X 또는 Y 좌표를 가진 점을 클릭할 수 있습니다.',
          '서로 다른 두 점을 고정하면 그 교차점에 스냅할 수 있습니다 — 이렇게 하면 (한 핀의 X, 다른 핀의 Y) 위치에 정확히 점이 놓이며, 각도 추적과 함께 사용할 수도 있습니다.',
          '새 도형을 그리는 대신 기존 도형을 참조점으로 재배치하려면 도형을 선택하고 <code>align</code>을 실행하세요 — 원본 점을 클릭한 다음 대상 점을 클릭하면 이동하고, 두 번째 점 쌍을 추가하면 회전(및 선택적으로 크기 조정)도 함께 적용됩니다.',
          '언제든지 <strong>Escape</strong>를 눌러 핀을 지우거나 정렬을 취소하고 다시 시작할 수 있습니다.',
        ],
        links: [
          { label: '그리드 & 스냅', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align', slug: 'commands/align' },
        ],
      },
  ...
  vi: {
    ...
    guides: [
      ...
      },
      {
        ...
        intro: 'Tải lên tệp .ttf của riêng bạn và sử dụng nó trong nhãn Text và Multileader — phông chữ vẫn được lưu trong trình duyệt của bạn cho các bản vẽ sau này.',
        steps: [
          'Gõ <code>FontManager</code> trong terminal để mở hộp thoại trực tiếp — hoặc, nếu bạn đã mở trình soạn thảo văn bản, nhấp nút <strong>Font Manager</strong> trên thanh công cụ của nó.',
          'Nhấp <strong>Add Font</strong> ở cuối hộp thoại và chọn một tệp <code>.ttf</code> — chỉ hỗ trợ phông chữ TrueType.',
          'Tên tệp trở thành tên phông chữ trong nhóm <strong>User</strong> (tải lên <code>MyFont.ttf</code> sẽ thêm một phông chữ tên là <code>MyFont</code>).',
          'Để áp dụng phông chữ, đặt một nhãn văn bản mới (lệnh <code>text</code>) hoặc nhấp đúp vào một Text hoặc Multileader hiện có để mở trình soạn thảo văn bản.',
          'Chọn phông chữ mới từ danh sách để áp dụng — khi có văn bản được chọn, nó chỉ thay đổi các ký tự đó; khi không chọn, nó áp dụng cho toàn bộ nhãn.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Trình Soạn Thảo Văn Bản', slug: 'interface/text-editor' },
          { label: 'Lệnh Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Bắt điểm và căn chỉnh chính xác',
        intro: 'Kết hợp Lưới & Bắt Điểm, Vector Pins, và lệnh Align để định vị hình học mới một cách chính xác — không cần gõ tọa độ thủ công.',
        steps: [
          'Bật <strong>Grid</strong> và <strong>Snap</strong> trên thanh điều khiển để khóa con trỏ vào một lưới đều — khoảng cách tự điều chỉnh khi bạn thu phóng, và các điểm bắt trên hình học hiện có (điểm cuối, điểm giữa, giao điểm) vẫn được ưu tiên hơn lưới.',
          'Di chuột qua bất kỳ điểm bắt nào trong nửa giây để ghim nó bằng <strong>Vector Pins</strong> (bật theo mặc định) — chốt ghim sẽ chiếu ra các đường tham chiếu ngang và dọc dạng đứt nét, cho phép bạn nhấp vào một điểm có cùng tọa độ X hoặc Y chính xác, mà không cần vẽ đường dựng hình.',
          'Ghim hai điểm khác nhau để bắt vào giao điểm của chúng — thao tác này đặt một điểm chính xác tại (X của một ghim, Y của ghim kia), và cũng hoạt động cùng với theo dõi góc.',
          'Để định vị lại một hình có sẵn theo các điểm tham chiếu thay vì vẽ hình học mới, hãy chọn nó và chạy <code>align</code> — nhấp một điểm nguồn rồi một điểm đích để di chuyển nó, hoặc thêm cặp điểm thứ hai để đồng thời xoay (và tùy chọn thay đổi tỷ lệ) vào đúng vị trí.',
          'Nhấn <strong>Escape</strong> bất cứ lúc nào để xóa các ghim hoặc hủy căn chỉnh và bắt đầu lại.',
        ],
        links: [
          { label: 'Lưới & Bắt Điểm', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  th: {
    ...
    guides: [
      ...
      {
        title: 'เพิ่มฟอนต์กำหนดเอง',
        intro: 'อัปโหลดไฟล์ .ttf ของคุณเองและใช้ในป้ายกำกับ Text และ Multileader — ฟอนต์จะถูกบันทึกไว้ในเบราว์เซอร์ของคุณสำหรับภาพวาดในอนาคต',
        steps: [
          'พิมพ์ <code>FontManager</code> ในเทอร์มินัลเพื่อเปิดกล่องโต้ตอบโดยตรง — หรือหากคุณเปิด text editor อยู่แล้ว ให้คลิกปุ่ม <strong>Font Manager</strong> บนแถบเครื่องมือของมัน',
          'คลิก <strong>Add Font</strong> ที่ส่วนท้ายของกล่องโต้ตอบและเลือกไฟล์ <code>.ttf</code> — รองรับเฉพาะฟอนต์ TrueType เท่านั้น',
          'ชื่อไฟล์จะกลายเป็นชื่อฟอนต์ในกลุ่ม <strong>User</strong> (การอัปโหลด <code>MyFont.ttf</code> จะเพิ่มฟอนต์ชื่อ <code>MyFont</code>)',
          'หากต้องการใช้ฟอนต์ ให้วางป้ายข้อความใหม่ (คำสั่ง <code>text</code>) หรือดับเบิลคลิกที่ Text หรือ Multileader ที่มีอยู่เพื่อเปิด text editor',
          'เลือกฟอนต์ใหม่จากรายการเพื่อใช้งาน — หากมีการเลือกข้อความไว้ จะเปลี่ยนเฉพาะตัวอักษรเหล่านั้น หากไม่ได้เลือก จะกำหนดทั้งป้ายกำกับ',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'text editor', slug: 'interface/text-editor' },
          { label: 'คำสั่ง Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'สแนปและจัดแนวอย่างแม่นยำ',
        intro: 'ผสมผสาน Grid & Snap, Vector Pins และคำสั่ง Align เพื่อวางตำแหน่งเรขาคณิตใหม่ได้อย่างแม่นยำ — โดยไม่ต้องพิมพ์พิกัดด้วยมือ',
        steps: [
          'เปิด <strong>Grid</strong> และ <strong>Snap</strong> ในแถบควบคุมเพื่อล็อกเคอร์เซอร์เข้ากับกริดปกติ — ระยะห่างจะปรับตามการซูม และการสแนปกับเรขาคณิตที่มีอยู่ (จุดปลาย จุดกึ่งกลาง จุดตัด) ยังคงมีความสำคัญเหนือกริด',
          'วางเมาส์ไว้เหนือจุดสแนปใดๆ ครึ่งวินาทีเพื่อปักหมุดด้วย <strong>Vector Pins</strong> (เปิดใช้งานเป็นค่าเริ่มต้น) — หมุดจะฉายเส้นอ้างอิงแนวนอนและแนวตั้งแบบเส้นประ ทำให้คุณคลิกจุดที่มีพิกัด X หรือ Y เดียวกันได้โดยไม่ต้องวาดเส้นช่วยสร้าง',
          'ปักหมุดสองจุดที่ต่างกันเพื่อสแนปไปยังจุดตัดของหมุดทั้งสอง — วิธีนี้จะวางจุดไว้ที่ตำแหน่ง (X ของหมุดหนึ่ง, Y ของอีกหมุด) พอดี และยังใช้งานร่วมกับการติดตามมุมได้ด้วย',
          'หากต้องการย้ายรูปทรงที่มีอยู่ไปยังจุดอ้างอิงแทนการวาดเรขาคณิตใหม่ ให้เลือกรูปทรงนั้นแล้วรันคำสั่ง <code>align</code> — คลิกจุดต้นทางแล้วคลิกจุดปลายทางเพื่อย้ายมัน หรือเพิ่มคู่จุดที่สองเพื่อหมุน (และปรับขนาดได้ตามต้องการ) ไปยังตำแหน่งนั้นด้วย',
          'กด <strong>Escape</strong> ได้ทุกเมื่อเพื่อล้างหมุดหรือยกเลิกการจัดแนวแล้วเริ่มใหม่',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  ms: {
    ...
    guides: [
      ...
      {
        title: 'Menambah fon kustom',
        intro: 'Muat naik fail .ttf anda sendiri dan gunakannya dalam label Text dan Multileader — fon kekal disimpan dalam pelayar anda untuk lukisan masa hadapan.',
        steps: [
          'Taip <code>FontManager</code> dalam terminal untuk membuka dialog secara terus — atau, jika anda sudah membuka text editor, klik butang <strong>Font Manager</strong> pada bar alatnya.',
          'Klik <strong>Add Font</strong> pada footer dialog dan pilih fail <code>.ttf</code> — hanya fon TrueType disokong.',
          'Nama fail menjadi nama fon dalam kumpulan <strong>User</strong> (memuat naik <code>MyFont.ttf</code> menambah fon bernama <code>MyFont</code>).',
          'Untuk menggunakan fon, letakkan label teks baharu (arahan <code>text</code>) atau klik dua kali pada Text atau Multileader sedia ada untuk membuka text editor.',
          'Pilih fon baharu daripada senarai untuk menggunakannya — dengan pemilihan teks ia hanya menggantikan aksara tersebut, tanpa pemilihan ia menetapkan keseluruhan label.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Text editor', slug: 'interface/text-editor' },
          { label: 'Arahan Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Snap dan jajarkan dengan tepat',
        intro: 'Gabungkan Grid & Snap, Vector Pins, dan arahan Align untuk meletakkan geometri baharu dengan tepat — tanpa menaip koordinat secara manual.',
        steps: [
          'Hidupkan <strong>Grid</strong> dan <strong>Snap</strong> pada bar kawalan untuk mengunci kursor kepada grid tetap — jarak menyesuaikan diri semasa anda zum, dan snap kepada geometri sedia ada (titik akhir, titik tengah, persilangan) masih diutamakan berbanding grid.',
          'Hoverkan kursor pada mana-mana titik snap selama setengah saat untuk menyematkannya dengan <strong>Vector Pins</strong> (dihidupkan secara lalai) — pin memancarkan garis rujukan mendatar dan menegak bertitik-titik, membolehkan anda mengklik titik yang berkongsi koordinat X atau Y yang sama tepat, tanpa melukis garis pembinaan.',
          'Sematkan dua titik berbeza untuk snap kepada persilangannya — ini meletakkan titik tepat pada (X satu pin, Y pin lain), dan turut berfungsi bersama penjejakan sudut.',
          'Untuk meletakkan semula bentuk sedia ada pada titik rujukan berbanding melukis geometri baharu, pilih bentuk itu dan jalankan <code>align</code> — klik titik sumber kemudian titik destinasi untuk menggerakkannya, atau tambah pasangan titik kedua untuk turut memutarkan (dan secara pilihan mengskalakan) ke kedudukannya.',
          'Tekan <strong>Escape</strong> pada bila-bila masa untuk mengosongkan pin atau membatalkan penjajaran dan mula semula.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  bn: {
    ...
    guides: [
      ...
      {
        title: 'কাস্টম ফন্ট যোগ করা',
        intro: 'আপনার নিজের .ttf ফাইল আপলোড করুন এবং Text ও Multileader লেবেলে ব্যবহার করুন — ফন্টটি ভবিষ্যতের অঙ্কনের জন্য আপনার ব্রাউজারে সংরক্ষিত থাকে।',
        steps: [
          'ডায়ালগ সরাসরি খুলতে টার্মিনালে <code>FontManager</code> টাইপ করুন — অথবা, যদি আপনার টেক্সট এডিটর ইতিমধ্যে খোলা থাকে, তাহলে এর টুলবারে <strong>Font Manager</strong> বাটনে ক্লিক করুন।',
          'ডায়ালগের ফুটারে <strong>Add Font</strong>-এ ক্লিক করুন এবং একটি <code>.ttf</code> ফাইল বেছে নিন — শুধুমাত্র TrueType ফন্ট সমর্থিত।',
          'ফাইলের নাম <strong>User</strong> গ্রুপে ফন্টের নাম হয়ে যায় (<code>MyFont.ttf</code> আপলোড করলে <code>MyFont</code> নামে একটি ফন্ট যোগ হয়)।',
          'ফন্ট প্রয়োগ করতে, একটি নতুন টেক্সট লেবেল স্থাপন করুন (<code>text</code> কমান্ড) অথবা টেক্সট এডিটর খুলতে বিদ্যমান কোনো Text বা Multileader-এ ডাবল-ক্লিক করুন।',
          'তালিকা থেকে নতুন ফন্ট নির্বাচন করে প্রয়োগ করুন — টেক্সট নির্বাচিত থাকলে শুধু সেই অক্ষরগুলোতে প্রয়োগ হয়, নির্বাচন না থাকলে পুরো লেবেলে প্রয়োগ হয়।',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'টেক্সট এডিটর', slug: 'interface/text-editor' },
          { label: 'Text কমান্ড', slug: 'commands/text' },
        ],
      },
      {
        title: 'নির্ভুলভাবে স্ন্যাপ ও অ্যালাইন করা',
        intro: 'হাতে কোঅর্ডিনেট টাইপ না করেই নতুন জ্যামিতিকে নির্ভুলভাবে স্থাপন করতে Grid & Snap, Vector Pins এবং Align কমান্ড একসাথে ব্যবহার করুন।',
        steps: [
          'কার্সরকে একটি নিয়মিত গ্রিডে লক করতে কন্ট্রোল বারে <strong>Grid</strong> এবং <strong>Snap</strong> চালু করুন — জুম করার সময় স্পেসিং স্বয়ংক্রিয়ভাবে সামঞ্জস্য হয়, এবং বিদ্যমান জ্যামিতির স্ন্যাপ (এন্ডপয়েন্ট, মিডপয়েন্ট, ইন্টারসেকশন) এখনও গ্রিডের চেয়ে অগ্রাধিকার পায়।',
          'কোনো স্ন্যাপ পয়েন্টের উপর আধা সেকেন্ড কার্সর রেখে <strong>Vector Pins</strong> (ডিফল্টরূপে চালু) দিয়ে সেটি পিন করুন — পিনটি ড্যাশড অনুভূমিক ও উল্লম্ব রেফারেন্স লাইন প্রক্ষেপণ করে, যাতে আপনি কোনো কনস্ট্রাকশন লাইন না এঁকেই একই X বা Y কোঅর্ডিনেট শেয়ার করা পয়েন্টে ক্লিক করতে পারেন।',
          'দুটি ভিন্ন পয়েন্ট পিন করে তাদের ইন্টারসেকশনে স্ন্যাপ করুন — এটি ঠিক (একটি পিনের X, অন্যটির Y) অবস্থানে একটি পয়েন্ট স্থাপন করে, এবং এটি অ্যাঙ্গেল ট্র্যাকিংয়ের সাথেও কাজ করে।',
          'নতুন জ্যামিতি আঁকার পরিবর্তে বিদ্যমান কোনো আকৃতিকে রেফারেন্স পয়েন্টে পুনঃস্থাপন করতে, সেটি নির্বাচন করুন এবং <code>align</code> চালান — এটিকে সরাতে একটি সোর্স পয়েন্ট তারপর একটি ডেস্টিনেশন পয়েন্ট ক্লিক করুন, অথবা এটিকে ঘোরাতে (এবং ঐচ্ছিকভাবে স্কেল করতে) দ্বিতীয় পয়েন্ট জোড়া যোগ করুন।',
          'পিন মুছতে বা অ্যালাইনমেন্ট বাতিল করে আবার শুরু করতে যেকোনো সময় <strong>Escape</strong> চাপুন।',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  sw: {
    ...
    guides: [
      ...
      {
        title: 'Kuongeza fonti maalum',
        intro: 'Pakia faili yako mwenyewe ya .ttf na uitumie katika lebo za Text na Multileader — fonti inabaki imehifadhiwa kwenye kivinjari chako kwa michoro ijayo.',
        steps: [
          'Andika <code>FontManager</code> kwenye terminal ili kufungua mazungumzo moja kwa moja — au, ikiwa tayari umefungua text editor, bofya kitufe cha <strong>Font Manager</strong> kwenye upau wake wa zana.',
          'Bofya <strong>Add Font</strong> chini ya mazungumzo na uchague faili ya <code>.ttf</code> — fonti za TrueType pekee ndizo zinazotumika.',
          'Jina la faili linakuwa jina la fonti kwenye kikundi cha <strong>User</strong> (kupakia <code>MyFont.ttf</code> huongeza fonti iitwayo <code>MyFont</code>).',
          'Ili kutumia fonti, weka lebo mpya ya maandishi (amri ya <code>text</code>) au bofya mara mbili Text au Multileader iliyopo ili kufungua text editor.',
          'Chagua fonti mpya kwenye orodha ili kuitumia — ikiwa maandishi yamechaguliwa, inatumika kwa herufi hizo pekee; bila uteuzi, inatumika kwa lebo nzima.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Kihariri cha maandishi', slug: 'interface/text-editor' },
          { label: 'Amri ya Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Kubandika na kupatanisha kwa usahihi',
        intro: 'Changanya Grid & Snap, Vector Pins, na amri ya Align kuweka jiometri mpya kwa usahihi — bila kuandika kuratibu kwa mkono.',
        steps: [
          'Washa <strong>Grid</strong> na <strong>Snap</strong> kwenye upau wa udhibiti ili kufunga kishale kwenye gridi ya kawaida — nafasi hurekebishwa unapokuza, na ubandikaji kwa jiometri iliyopo (ncha, katikati, makutano) bado unapewa kipaumbele zaidi ya gridi.',
          'Weka kishale juu ya sehemu yoyote ya kubandika kwa sekunde nusu ili kuibandika kwa <strong>Vector Pins</strong> (imewashwa kwa chaguo-msingi) — pini hutoa mistari ya rejeleo ya mlalo na wima yenye vistari, ili uweze kubofya sehemu inayoshiriki kuratibu X au Y sawa kabisa, bila kuchora mstari wa ujenzi.',
          'Bandika sehemu mbili tofauti ili kubandika kwenye makutano yao — hii huweka sehemu kwa usahihi kwenye (X ya pini moja, Y ya nyingine), na hufanya kazi pamoja na ufuatiliaji wa pembe pia.',
          'Ili kuweka upya umbo lililopo kwenye sehemu za rejeleo badala ya kuchora jiometri mpya, lichague na uendeshe <code>align</code> — bofya sehemu ya chanzo kisha sehemu ya lengo ili kulihamisha, au ongeza jozi ya pili ya sehemu ili pia kulizungusha (na kwa hiari kulibadilisha ukubwa) mahali pake.',
          'Bonyeza <strong>Escape</strong> wakati wowote kufuta pini au kughairi upatanishaji na kuanza upya.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  ur: {
    ...
    guides: [
      ...
      {
        title: 'کسٹم فونٹ شامل کریں',
        intro: 'اپنی .ttf فائل اپ لوڈ کریں اور اسے Text اور Multileader لیبلز میں استعمال کریں — فونٹ آپ کے براؤزر میں محفوظ رہتا ہے تاکہ آپ اسے آئندہ ڈرائنگز میں بھی استعمال کر سکیں۔',
        steps: [
          'ڈائیلاگ کو براہ راست کھولنے کے لیے ٹرمینل میں <code>FontManager</code> ٹائپ کریں — یا، اگر آپ کے پاس پہلے سے text editor کھلا ہے، تو اس کے ٹول بار میں <strong>Font Manager</strong> بٹن پر کلک کریں۔',
          'ڈائیلاگ کے نچلے حصے میں <strong>Add Font</strong> پر کلک کریں اور ایک <code>.ttf</code> فائل منتخب کریں — صرف TrueType فونٹس سپورٹڈ ہیں۔',
          'فائل کا نام <strong>User</strong> گروپ میں فونٹ کا نام بن جاتا ہے (<code>MyFont.ttf</code> اپ لوڈ کرنے سے <code>MyFont</code> نامی فونٹ شامل ہو جاتا ہے)۔',
          'فونٹ لاگو کرنے کے لیے، ایک نیا ٹیکسٹ لیبل رکھیں (<code>text</code> کمانڈ) یا موجودہ Text یا Multileader پر ڈبل کلک کر کے text editor کھولیں۔',
          'اسے لاگو کرنے کے لیے فہرست سے نیا فونٹ منتخب کریں — اگر متن منتخب ہو تو صرف انہی حروف پر لاگو ہوتا ہے، ورنہ پورے لیبل پر لاگو ہوتا ہے۔',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'ٹیکسٹ ایڈیٹر', slug: 'interface/text-editor' },
          { label: 'Text کمانڈ', slug: 'commands/text' },
        ],
      },
      {
        title: 'درست طریقے سے سنیپ اور الائن کرنا',
        intro: 'نئی جیومیٹری کو ہاتھ سے کوآرڈینیٹس ٹائپ کیے بغیر بالکل درست جگہ پر رکھنے کے لیے Grid & Snap، Vector Pins، اور Align کمانڈ کو ایک ساتھ استعمال کریں۔',
        steps: [
          'کرسر کو باقاعدہ گرڈ پر لاک کرنے کے لیے کنٹرول بار میں <strong>Grid</strong> اور <strong>Snap</strong> آن کریں — زوم کرنے پر فاصلہ خود بخود ایڈجسٹ ہوتا ہے، اور موجودہ جیومیٹری کے سنیپس (اینڈ پوائنٹ، مڈ پوائنٹ، انٹرسیکشن) اب بھی گرڈ پر ترجیح رکھتے ہیں۔',
          'کسی بھی سنیپ پوائنٹ پر آدھے سیکنڈ کے لیے کرسر رکھ کر اسے <strong>Vector Pins</strong> (بطور ڈیفالٹ آن) سے پن کریں — پن ڈیشڈ افقی اور عمودی حوالہ لائنیں پروجیکٹ کرتا ہے، تاکہ آپ بغیر کنسٹرکشن لائن کھینچے اسی X یا Y کوآرڈینیٹ والے پوائنٹ پر کلک کر سکیں۔',
          'دو مختلف پوائنٹس کو پن کر کے ان کے انٹرسیکشن پر سنیپ کریں — اس سے پوائنٹ بالکل (ایک پن کا X، دوسرے کا Y) پر رکھا جاتا ہے، اور یہ اینگل ٹریکنگ کے ساتھ بھی کام کرتا ہے۔',
          'نئی جیومیٹری بنانے کے بجائے کسی موجودہ شکل کو حوالہ پوائنٹس پر دوبارہ رکھنے کے لیے، اسے منتخب کریں اور <code>align</code> چلائیں — اسے حرکت دینے کے لیے ایک سورس پوائنٹ پھر ایک ڈیسٹینیشن پوائنٹ پر کلک کریں، یا اسے گھمانے (اور اختیاری طور پر اسکیل کرنے) کے لیے دوسرا پوائنٹ جوڑا شامل کریں۔',
          'پنز صاف کرنے یا ترتیب منسوخ کر کے دوبارہ شروع کرنے کے لیے کسی بھی وقت <strong>Escape</strong> دبائیں۔',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  el: {
    ...
    guides: [
      ...
      {
        title: 'Προσθήκη προσαρμοσμένης γραμματοσειράς',
        intro: 'Μεταφορτώστε το δικό σας αρχείο .ttf και χρησιμοποιήστε το σε ετικέτες Text και Multileader — η γραμματοσειρά παραμένει αποθηκευμένη στον browser σας για μελλοντικά σχέδια.',
        steps: [
          'Πληκτρολογήστε <code>FontManager</code> στο τερματικό για να ανοίξετε απευθείας το παράθυρο διαλόγου — ή, αν έχετε ήδη ανοιχτό τον επεξεργαστή κειμένου, κάντε κλικ στο κουμπί <strong>Font Manager</strong> στη γραμμή εργαλείων του.',
          'Κάντε κλικ στο <strong>Add Font</strong> στο υποσέλιδο του παραθύρου διαλόγου και επιλέξτε ένα αρχείο <code>.ttf</code> — υποστηρίζονται μόνο γραμματοσειρές TrueType.',
          'Το όνομα του αρχείου γίνεται το όνομα της γραμματοσειράς στην ομάδα <strong>User</strong> (η μεταφόρτωση του <code>MyFont.ttf</code> προσθέτει μια γραμματοσειρά με όνομα <code>MyFont</code>).',
          'Για να εφαρμόσετε τη γραμματοσειρά, τοποθετήστε μια νέα ετικέτα κειμένου (εντολή <code>text</code>) ή κάντε διπλό κλικ σε ένα υπάρχον Text ή Multileader για να ανοίξετε τον επεξεργαστή κειμένου.',
          'Επιλέξτε τη νέα γραμματοσειρά από τη λίστα για να την εφαρμόσετε — με επιλεγμένο κείμενο εφαρμόζεται μόνο σε αυτούς τους χαρακτήρες, χωρίς επιλογή ορίζει ολόκληρη την ετικέτα.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Επεξεργαστής κειμένου', slug: 'interface/text-editor' },
          { label: 'Εντολή Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Ακριβής προσκόλληση και στοίχιση',
        intro: 'Συνδυάστε το Grid & Snap, τα Vector Pins και την εντολή Align για να τοποθετήσετε τη νέα γεωμετρία με ακρίβεια — χωρίς να πληκτρολογείτε συντεταγμένες με το χέρι.',
        steps: [
          'Ενεργοποιήστε τα <strong>Grid</strong> και <strong>Snap</strong> στη γραμμή ελέγχου για να κλειδώσετε τον δρομέα σε ένα κανονικό πλέγμα — το διάστημα προσαρμόζεται καθώς κάνετε ζουμ, και οι προσκολλήσεις σε υπάρχουσα γεωμετρία (άκρο, μέσο, τομή) εξακολουθούν να έχουν προτεραιότητα έναντι του πλέγματος.',
          'Αιωρήστε τον δρομέα πάνω από οποιοδήποτε σημείο προσκόλλησης για μισό δευτερόλεπτο για να το καρφιτσώσετε με τα <strong>Vector Pins</strong> (ενεργά από προεπιλογή) — η καρφίτσα προβάλλει διακεκομμένες οριζόντιες και κάθετες γραμμές αναφοράς, ώστε να μπορείτε να κάνετε κλικ σε ένα σημείο που μοιράζεται ακριβώς την ίδια συντεταγμένη X ή Y, χωρίς να σχεδιάσετε βοηθητική γραμμή.',
          'Καρφιτσώστε δύο διαφορετικά σημεία για να προσκολληθείτε στην τομή τους — αυτό τοποθετεί ένα σημείο ακριβώς στο (X μιας καρφίτσας, Y της άλλης), και λειτουργεί επίσης μαζί με την παρακολούθηση γωνίας.',
          'Για να επανατοποθετήσετε ένα υπάρχον σχήμα σε σημεία αναφοράς αντί να σχεδιάσετε νέα γεωμετρία, επιλέξτε το και εκτελέστε <code>align</code> — κάντε κλικ σε ένα σημείο προέλευσης και έπειτα σε ένα σημείο προορισμού για να το μετακινήσετε, ή προσθέστε ένα δεύτερο ζεύγος για να το περιστρέψετε επίσης (και προαιρετικά να το κλιμακώσετε) στη θέση του.',
          'Πατήστε <strong>Escape</strong> ανά πάσα στιγμή για να καθαρίσετε τις καρφίτσες ή να ακυρώσετε τη στοίχιση και να ξεκινήσετε από την αρχή.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  pa: {
    ...
    guides: [
      ...
      {
        title: 'ਕਸਟਮ ਫੌਂਟ ਸ਼ਾਮਲ ਕਰਨਾ',
        intro: 'ਆਪਣੀ .ttf ਫਾਈਲ ਅੱਪਲੋਡ ਕਰੋ ਅਤੇ ਇਸਨੂੰ Text ਅਤੇ Multileader ਲੇਬਲਾਂ ਵਿੱਚ ਵਰਤੋ — ਫੌਂਟ ਭਵਿੱਖ ਦੀਆਂ ਡਰਾਇੰਗਾਂ ਲਈ ਤੁਹਾਡੇ ਬ੍ਰਾਊਜ਼ਰ ਵਿੱਚ ਸੇਵ ਰਹਿੰਦਾ ਹੈ।',
        steps: [
          'ਡਾਇਲਾਗ ਸਿੱਧਾ ਖੋਲ੍ਹਣ ਲਈ ਟਰਮੀਨਲ ਵਿੱਚ <code>FontManager</code> ਟਾਈਪ ਕਰੋ — ਜਾਂ, ਜੇ ਤੁਹਾਡੇ ਕੋਲ ਪਹਿਲਾਂ ਤੋਂ ਟੈਕਸਟ ਐਡੀਟਰ ਖੁੱਲ੍ਹਾ ਹੈ, ਤਾਂ ਇਸਦੇ ਟੂਲਬਾਰ ਵਿੱਚ <strong>Font Manager</strong> ਬਟਨ ਉੱਤੇ ਕਲਿੱਕ ਕਰੋ।',
          'ਡਾਇਲਾਗ ਦੇ ਫੁੱਟਰ ਵਿੱਚ <strong>Add Font</strong> ਉੱਤੇ ਕਲਿੱਕ ਕਰੋ ਅਤੇ ਇੱਕ <code>.ttf</code> ਫਾਈਲ ਚੁਣੋ — ਸਿਰਫ਼ TrueType ਫੌਂਟ ਸਮਰਥਿਤ ਹਨ।',
          'ਫਾਈਲ ਦਾ ਨਾਮ <strong>User</strong> ਗਰੁੱਪ ਵਿੱਚ ਫੌਂਟ ਦਾ ਨਾਮ ਬਣ ਜਾਂਦਾ ਹੈ (<code>MyFont.ttf</code> ਅੱਪਲੋਡ ਕਰਨ ਨਾਲ <code>MyFont</code> ਨਾਮ ਦਾ ਇੱਕ ਫੌਂਟ ਜੁੜ ਜਾਂਦਾ ਹੈ)।',
          'ਫੌਂਟ ਲਾਗੂ ਕਰਨ ਲਈ, ਇੱਕ ਨਵਾਂ ਟੈਕਸਟ ਲੇਬਲ ਰੱਖੋ (<code>text</code> ਕਮਾਂਡ) ਜਾਂ ਟੈਕਸਟ ਐਡੀਟਰ ਖੋਲ੍ਹਣ ਲਈ ਮੌਜੂਦਾ Text ਜਾਂ Multileader ਉੱਤੇ ਡਬਲ-ਕਲਿੱਕ ਕਰੋ।',
          'ਇਸਨੂੰ ਲਾਗੂ ਕਰਨ ਲਈ ਸੂਚੀ ਵਿੱਚੋਂ ਨਵਾਂ ਫੌਂਟ ਚੁਣੋ — ਟੈਕਸਟ ਚੁਣੇ ਹੋਣ ਤੇ ਇਹ ਸਿਰਫ਼ ਉਹਨਾਂ ਅੱਖਰਾਂ ਉੱਤੇ ਲਾਗੂ ਹੁੰਦਾ ਹੈ, ਬਿਨਾਂ ਚੋਣ ਦੇ ਇਹ ਪੂਰੇ ਲੇਬਲ ਉੱਤੇ ਲਾਗੂ ਹੁੰਦਾ ਹੈ।',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'ਟੈਕਸਟ ਐਡੀਟਰ', slug: 'interface/text-editor' },
          { label: 'Text ਕਮਾਂਡ', slug: 'commands/text' },
        ],
      },
      {
        title: 'ਸਹੀ ਸਨੈਪ ਅਤੇ ਅਲਾਈਨ ਕਰਨਾ',
        intro: 'ਨਵੀਂ ਜਿਓਮੈਟਰੀ ਨੂੰ ਹੱਥੀਂ ਕੋਆਰਡੀਨੇਟ ਟਾਈਪ ਕੀਤੇ ਬਿਨਾਂ ਬਿਲਕੁਲ ਸਹੀ ਥਾਂ ਤੇ ਰੱਖਣ ਲਈ Grid & Snap, Vector Pins, ਅਤੇ Align ਕਮਾਂਡ ਨੂੰ ਇਕੱਠੇ ਵਰਤੋ।',
        steps: [
          'ਕਰਸਰ ਨੂੰ ਇੱਕ ਨਿਯਮਤ ਗਰਿੱਡ ਨਾਲ ਲਾਕ ਕਰਨ ਲਈ ਕੰਟਰੋਲ ਬਾਰ ਵਿੱਚ <strong>Grid</strong> ਅਤੇ <strong>Snap</strong> ਚਾਲੂ ਕਰੋ — ਜ਼ੂਮ ਕਰਨ ਤੇ ਸਪੇਸਿੰਗ ਆਪਣੇ ਆਪ ਅਡਜਸਟ ਹੋ ਜਾਂਦੀ ਹੈ, ਅਤੇ ਮੌਜੂਦਾ ਜਿਓਮੈਟਰੀ ਦੇ ਸਨੈਪ (ਐਂਡਪੁਆਇੰਟ, ਮਿਡਪੁਆਇੰਟ, ਇੰਟਰਸੈਕਸ਼ਨ) ਅਜੇ ਵੀ ਗਰਿੱਡ ਨਾਲੋਂ ਪਹਿਲ ਰੱਖਦੇ ਹਨ।',
          'ਕਿਸੇ ਵੀ ਸਨੈਪ ਪੁਆਇੰਟ ਤੇ ਅੱਧੇ ਸਕਿੰਟ ਲਈ ਕਰਸਰ ਰੱਖ ਕੇ ਇਸਨੂੰ <strong>Vector Pins</strong> (ਮੂਲ ਰੂਪ ਵਿੱਚ ਚਾਲੂ) ਨਾਲ ਪਿੰਨ ਕਰੋ — ਪਿੰਨ ਡੈਸ਼ਡ ਖਿਤਿਜੀ ਅਤੇ ਲੰਬਕਾਰੀ ਹਵਾਲਾ ਲਾਈਨਾਂ ਪ੍ਰੋਜੈਕਟ ਕਰਦਾ ਹੈ, ਤਾਂ ਜੋ ਤੁਸੀਂ ਬਿਨਾਂ ਕੰਸਟਰੱਕਸ਼ਨ ਲਾਈਨ ਖਿੱਚੇ ਉਸੇ X ਜਾਂ Y ਕੋਆਰਡੀਨੇਟ ਵਾਲੇ ਪੁਆਇੰਟ ਤੇ ਕਲਿੱਕ ਕਰ ਸਕੋ।',
          'ਦੋ ਵੱਖ-ਵੱਖ ਪੁਆਇੰਟਾਂ ਨੂੰ ਪਿੰਨ ਕਰਕੇ ਉਹਨਾਂ ਦੇ ਇੰਟਰਸੈਕਸ਼ਨ ਤੇ ਸਨੈਪ ਕਰੋ — ਇਹ ਪੁਆਇੰਟ ਨੂੰ ਬਿਲਕੁਲ (ਇੱਕ ਪਿੰਨ ਦਾ X, ਦੂਜੇ ਦਾ Y) ਤੇ ਰੱਖਦਾ ਹੈ, ਅਤੇ ਇਹ ਐਂਗਲ ਟਰੈਕਿੰਗ ਨਾਲ ਵੀ ਕੰਮ ਕਰਦਾ ਹੈ।',
          'ਨਵੀਂ ਜਿਓਮੈਟਰੀ ਬਣਾਉਣ ਦੀ ਬਜਾਏ ਕਿਸੇ ਮੌਜੂਦਾ ਆਕਾਰ ਨੂੰ ਹਵਾਲਾ ਪੁਆਇੰਟਾਂ ਤੇ ਮੁੜ-ਸਥਿਤ ਕਰਨ ਲਈ, ਇਸਨੂੰ ਚੁਣੋ ਅਤੇ <code>align</code> ਚਲਾਓ — ਇਸਨੂੰ ਹਿਲਾਉਣ ਲਈ ਇੱਕ ਸੋਰਸ ਪੁਆਇੰਟ ਫਿਰ ਇੱਕ ਡੈਸਟੀਨੇਸ਼ਨ ਪੁਆਇੰਟ ਕਲਿੱਕ ਕਰੋ, ਜਾਂ ਇਸਨੂੰ ਘੁਮਾਉਣ (ਅਤੇ ਵਿਕਲਪਿਕ ਤੌਰ ਤੇ ਸਕੇਲ ਕਰਨ) ਲਈ ਦੂਜਾ ਪੁਆਇੰਟ ਜੋੜਾ ਸ਼ਾਮਲ ਕਰੋ।',
          'ਪਿੰਨ ਸਾਫ਼ ਕਰਨ ਜਾਂ ਅਲਾਈਨਮੈਂਟ ਰੱਦ ਕਰਕੇ ਦੁਬਾਰਾ ਸ਼ੁਰੂ ਕਰਨ ਲਈ ਕਿਸੇ ਵੀ ਸਮੇਂ <strong>Escape</strong> ਦਬਾਓ।',
        ],
        links: [
          { label: 'ਗ੍ਰਿਡ ਅਤੇ ਸਨੈਪ', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align', slug: 'commands/align' },
        ],
      },
  ...
  sv: {
    ...
    guides: [
      ...
      {
        title: 'Lägg till ett anpassat typsnitt',
        intro: 'Ladda upp din egen .ttf-fil och använd den i Text- och Multileader-etiketter — typsnittet förblir sparat i din webbläsare för framtida ritningar.',
        steps: [
          'Skriv <code>FontManager</code> i terminalen för att öppna dialogrutan direkt — eller, om du redan har texteditorn öppen, klicka på knappen <strong>Font Manager</strong> i dess verktygsfält.',
          'Klicka på <strong>Add Font</strong> i dialogrutans sidfot och välj en <code>.ttf</code>-fil — endast TrueType-typsnitt stöds.',
          'Filnamnet blir typsnittets namn i gruppen <strong>User</strong> (att ladda upp <code>MyFont.ttf</code> lägger till ett typsnitt med namnet <code>MyFont</code>).',
          'För att tillämpa typsnittet, placera en ny textetikett (kommandot <code>text</code>) eller dubbelklicka på en befintlig Text eller Multileader för att öppna texteditorn.',
          'Välj det nya typsnittet i listan för att tillämpa det — med markerad text påverkas endast dessa tecken, utan markering sätts hela etiketten.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Textredigerare', slug: 'interface/text-editor' },
          { label: 'Text-kommandot', slug: 'commands/text' },
        ],
      },
      {
        title: 'Snäppa och justera precist',
        intro: 'Kombinera Rutnät & Snap, Vector Pins och kommandot Align för att positionera ny geometri exakt — utan att skriva in koordinater för hand.',
        steps: [
          'Slå på <strong>Grid</strong> och <strong>Snap</strong> i kontrollfältet för att låsa markören till ett regelbundet rutnät — avståndet anpassas när du zoomar, och snappning till befintlig geometri (ändpunkt, mittpunkt, skärningspunkt) har fortfarande företräde framför rutnätet.',
          'Håll muspekaren över en snappunkt i en halv sekund för att fästa den med <strong>Vector Pins</strong> (aktiverat som standard) — nålen projicerar streckade horisontella och vertikala referenslinjer, så att du kan klicka på en punkt som delar exakt samma X- eller Y-koordinat, utan att rita en hjälplinje.',
          'Fäst två olika punkter för att snappa till deras skärningspunkt — det placerar en punkt exakt vid (X för en nål, Y för den andra), och fungerar även tillsammans med vinkelspårning.',
          'För att flytta en befintlig form till referenspunkter istället för att rita ny geometri, markera den och kör <code>align</code> — klicka en källpunkt och sedan en målpunkt för att flytta den, eller lägg till ett andra punktpar för att även rotera (och eventuellt skala) den på plats.',
          'Tryck på <strong>Escape</strong> när som helst för att rensa nålar eller avbryta justeringen och börja om.',
        ],
        links: [
          { label: 'Rutnät & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
          { label: 'Align', slug: 'commands/align' },
        ],
      },
  ...
  tl: {
    ...
    guides: [
      ...
      {
        title: 'Magdagdag ng custom na font',
        intro: 'I-upload ang sarili mong .ttf file at gamitin ito sa mga Text at Multileader label — mananatiling naka-save ang font sa browser mo para sa mga susunod na drawing.',
        steps: [
          'I-type ang <code>FontManager</code> sa terminal para direktang buksan ang dialog — o, kung bukas na ang text editor mo, i-click ang <strong>Font Manager</strong> button sa toolbar nito.',
          'I-click ang <strong>Add Font</strong> sa footer ng dialog at pumili ng <code>.ttf</code> file — TrueType fonts lang ang suportado.',
          'Ang pangalan ng file ang magiging pangalan ng font sa grupong <strong>User</strong> (ang pag-upload ng <code>MyFont.ttf</code> ay magdaragdag ng font na pinangalanang <code>MyFont</code>).',
          'Para ilapat ang font, maglagay ng bagong text label (<code>text</code> command) o i-double-click ang isang umiiral na Text o Multileader para buksan ang text editor.',
          'Piliin ang bagong font sa listahan para i-apply ito — kapag may napiling text, ito lang ang mababago; kung walang napili, ang buong label ang mababago.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Text editor', slug: 'interface/text-editor' },
      ...
      {
        title: 'Tumpak na pag-snap at pag-align',
        intro: 'Pagsamahin ang Grid & Snap, Vector Pins, at ang Align command para tumpak na mailagay ang bagong geometry — nang hindi nagta-type ng coordinates nang manu-mano.',
        steps: [
          'I-on ang <strong>Grid</strong> at <strong>Snap</strong> sa control bar para i-lock ang cursor sa regular na grid — ang spacing ay awtomatikong nagbabago habang nagzo-zoom ka, at ang mga snap sa umiiral na geometry (endpoint, midpoint, intersection) ay nananatiling mas prayoridad kaysa sa grid.',
          'I-hover ang cursor sa anumang snap point nang kalahating segundo para i-pin ito gamit ang <strong>Vector Pins</strong> (naka-on bilang default) — nagpo-project ang pin ng dashed na pahalang at patayong reference line, para makapag-click ka ng point na may eksaktong parehong X o Y coordinate, nang hindi gumagawa ng construction line.',
          'I-pin ang dalawang magkaibang point para mag-snap sa intersection nila — inilalagay nito ang isang point nang eksakto sa (X ng isang pin, Y ng isa pa), at gumagana rin kasama ang angle tracking.',
          'Para ilipat ang isang umiiral na hugis papunta sa mga reference point sa halip na gumuhit ng bagong geometry, piliin ito at patakbuhin ang <code>align</code> — i-click ang isang source point pagkatapos ang destination point para ilipat ito, o magdagdag ng pangalawang pares para i-rotate rin (at opsyonal na i-scale) ito sa tamang posisyon.',
          'Pindutin ang <strong>Escape</strong> anumang oras para burahin ang mga pin o kanselahin ang alignment at magsimulang muli.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  nl: {
    ...
    guides: [
      ...
      {
        title: 'Een aangepast lettertype toevoegen',
        intro: 'Upload uw eigen .ttf-bestand en gebruik het in Text- en Multileader-labels — het lettertype blijft opgeslagen in uw browser voor toekomstige tekeningen.',
        steps: [
          'Typ <code>FontManager</code> in de terminal om het dialoogvenster direct te openen — of, als u de teksteditor al open heeft, klik op de knop <strong>Font Manager</strong> in de werkbalk ervan.',
          'Klik op <strong>Add Font</strong> onderaan het dialoogvenster en kies een <code>.ttf</code>-bestand — alleen TrueType-lettertypen worden ondersteund.',
          'De bestandsnaam wordt de naam van het lettertype in de groep <strong>User</strong> (het uploaden van <code>MyFont.ttf</code> voegt een lettertype toe met de naam <code>MyFont</code>).',
          'Om het lettertype toe te passen, plaatst u een nieuw tekstlabel (<code>text</code>-commando) of dubbelklikt u op een bestaande Text of Multileader om de teksteditor te openen.',
          'Selecteer het nieuwe lettertype in de lijst om het toe te passen — met een tekstselectie geldt het alleen voor die tekens, zonder selectie wordt het hele label ingesteld.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Tekstverwerker', slug: 'interface/text-editor' },
          { label: 'Text-commando', slug: 'commands/text' },
        ],
      },
      {
        title: 'Nauwkeurig uitlijnen met snap',
        intro: 'Combineer Raster & Snap, Vector Pins en het Align-commando om nieuwe geometrie exact te positioneren — zonder handmatig coördinaten in te typen.',
        steps: [
          'Schakel <strong>Grid</strong> en <strong>Snap</strong> in de bedieningsbalk in om de cursor aan een regelmatig raster te vergrendelen — de afstand past zich aan tijdens het zoomen, en vangpunten op bestaande geometrie (eindpunt, middelpunt, snijpunt) blijven voorrang hebben boven het raster.',
          'Beweeg de cursor een halve seconde over een vangpunt om het vast te zetten met <strong>Vector Pins</strong> (standaard ingeschakeld) — de pin projecteert gestippelde horizontale en verticale referentielijnen, zodat u een punt kunt aanklikken met exact dezelfde X- of Y-coördinaat, zonder een hulplijn te tekenen.',
          'Zet twee verschillende punten vast om naar hun snijpunt te snappen — dit plaatst een punt precies op (X van de ene pin, Y van de andere), en werkt ook samen met hoektracking.',
          'Om een bestaande vorm naar referentiepunten te verplaatsen in plaats van nieuwe geometrie te tekenen, selecteert u de vorm en voert u <code>align</code> uit — klik een bronpunt en vervolgens een doelpunt om de vorm te verplaatsen, of voeg een tweede puntenpaar toe om deze ook te roteren (en optioneel te schalen) naar de juiste plek.',
          'Druk op elk moment op <strong>Escape</strong> om pins te wissen of de uitlijning te annuleren en opnieuw te beginnen.',
        ],
        links: [
          { label: 'Raster & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  he: {
    title: 'איך להשתמש ב-KulmanLab — מדריכים למשימות CAD נפוצות',
    desc: 'מדריכים שלב-אחר-שלב ל-KulmanLab CAD: פתיחת קבצי DXF, שרטוט במידות מדויקות, שימוש בשכבות, הוספת מידות, הכנת קבצים לחיתוך לייזר, הדפסה וגיבוי השרטוטים שלכם.',
    heading: 'מדריכים',
    subtitle: 'מדריכים שלב-אחר-שלב למשימות הנפוצות ביותר ב-KulmanLab CAD.',
    docsLabel: 'תיעוד:',
    outroTitle: 'מחפשים פקודה ספציפית?',
    outroHtml: '<a href="/he/docs/">מדריך הפקודות</a> מתעד את כל 50+ הפקודות, וה<a href="/he/faq/">שאלות נפוצות</a> מכסות אחסון, פורמטים ותאימות.',
    guides: [
      {
        title: 'פתיחת קובץ DXF',
        intro: 'KulmanLab קורא DXF — פורמט החילופין שכל כלי CAD יכול להפיק. הקבצים נפתחים ישירות מהדיסק שלכם ולעולם לא עוזבים את המכשיר.',
        steps: [
          'פתחו את <a href="https://app.kulmanlab.com">app.kulmanlab.com</a>.',
          'לחצו על כפתור <strong>Import</strong> בפאנל File, או הקלידו <code>import</code> במסוף.',
          'בחרו קובץ <code>.dxf</code> (או <code>.json</code> של KulmanLab) מהמחשב שלכם.',
          'השרטוט נטען לקנבס ונשמר אוטומטית באחסון הדפדפן שלכם, כך שהוא יישאר שם גם בפעם הבאה.',
        ],
        links: [
      ...
      {
        title: 'שרטוט במידות מדויקות',
        intro: 'כל פקודת שרטוט מקבלת קלט מוקלד, כך שלעולם לא תצטרכו להעריך אורך או קואורדינטה במבט.',
        steps: [
          'התחילו פקודה — לחצו על <strong>Line</strong> או הקלידו <code>line</code> ולחצו <strong>Enter</strong>.',
          'הקלידו נקודת התחלה כ-<code>x,y</code> (לדוגמה <code>0,0</code>) ולחצו <strong>Enter</strong>.',
          'הזיזו את הסמן לכיוון הרצוי, ואז הקלידו אורך (לדוגמה <code>250</code>) ולחצו <strong>Enter</strong> — הקטע יישורטט באורך מדויק זה.',
          'הפעילו נעילת זווית בשורת הבקרה (או החזיקו את המרווח המוגדר) כדי לשמור על קטעים בזוויות מדויקות כמו 30°, 45° או 90°.',
        ],
        links: [
          { label: 'פקודת Line', slug: 'commands/line' },
          { label: 'רשת והצמדה', slug: 'interface/grid-snap' },
        ],
      },
      {
        title: 'ארגון שרטוט באמצעות שכבות',
        intro: 'שכבות שומרות על קווי בנייה, גיאומטריה והערות נפרדים — והן שורדות ייצוא DXF.',
        steps: [
          'פתחו את פאנל <strong>Layers</strong> בסרגל הכלים וצרו שכבה לכל סוג תוכן (מתאר, מידות, הערות...).',
          'הגדירו צבע, סוג קו ועובי קו לכל שכבה כך שישויות יקבלו ברירות מחדל הגיוניות.',
          'הפכו שכבה לפעילה עם <code>layer-make-current</code> לפני השרטוט, או הזיזו ישויות נבחרות עם <code>layer-match</code>.',
          'השתמשו ב-<code>layer-isolate</code> כדי להסתיר הכול חוץ מהשכבה שעליה אתם עובדים, וב-<code>layer-unfreeze-all</code> כדי להחזיר הכול.',
        ],
        links: [
          { label: 'בידוד שכבה', slug: 'commands/layer-isolate' },
          { label: 'סוג קו', slug: 'interface/linetype' },
          { label: 'עובי קו', slug: 'interface/lineweight' },
        ],
      },
      {
        title: 'הוספת מידות',
        intro: 'מידות הן ישויות DIMENSION אמיתיות של DXF, כך שהן עוברות ללא פגיעה לכל כלי CAD אחר.',
        steps: [
          'הקלידו <code>dim-linear</code> (אופקי/אנכי) או <code>dim-aligned</code> (מקביל לקצה הנמדד) במסוף.',
          'בחרו את שתי הנקודות שברצונכם למדוד, ולאחר מכן מקמו את קו המידה בלחיצה שלישית.',
          'שרשרו שורת מדידות עם <code>dim-continue</code> — כל מידה חדשה מתחילה במקום שהקודמת הסתיימה.',
          'עבור מעגלים וקשתות, השתמשו ב-<code>dim-radius</code>, <code>dim-diameter</code> או <code>dim-angular</code>.',
          'לחצו לחיצה כפולה על כל תווית מידה כדי לערוך את הטקסט שלה.',
        ],
        links: [
          { label: 'מידה ליניארית', slug: 'commands/dim-linear' },
          { label: 'מידה רציפה', slug: 'commands/dim-continue' },
          { label: 'עורך טקסט', slug: 'interface/text-editor' },
        ],
      },
      {
        title: 'מדידת מרחק, זווית ושטח',
        intro: 'מדידות מהירות בלי ליצור גיאומטריה כלשהי — התוצאות נשארות על המסך עד שתלחצו Escape.',
        steps: [
          'הקלידו <code>distance</code> ובחרו שתי נקודות כדי לקרוא אורך.',
          'הקלידו <code>angle</code> ובחרו שני קווים (או שלוש נקודות) כדי לקרוא את הזווית ביניהם.',
          'הקלידו <code>area</code> ולחצו על שלוש נקודות או יותר, ואז לחצו <strong>Enter</strong> — השטח התחום וההיקף יוצגו.',
        ],
        links: [
          { label: 'מרחק', slug: 'commands/distance' },
          { label: 'זווית', slug: 'commands/angle' },
          { label: 'שטח', slug: 'commands/area' },
        ],
      },
      {
        title: 'הכנת קובץ לחיתוך לייזר או CNC',
        intro: 'תהליך העבודה ש-KulmanLab נבנה עבורו במקור: בדקו קובץ, נקו אותו, שלחו אותו למכונה.',
        steps: [
          'ייבאו את קובץ ה-DXF ועברו עליו — <code>fit</code> מכניס את כל השרטוט לתצוגה.',
          'מחקו כל דבר שהמכונה לא אמורה לחתוך: קווי בנייה, הערות, מידות. <code>layer-isolate</code> עוזר למצוא רכיבים תועים.',
          'סדרו את הגיאומטריה: <code>trim</code> קצוות בולטים, סגרו רווחים, ובדקו גדלים עם <code>distance</code>.',
          'ייצאו כ-DXF וטענו לתוכנת המכונה שלכם. נתיבי החיתוך נשמרים בדיוק כפי שנשרטטו — KulmanLab כותב DXF פשוט בפורמט AC1032.',
        ],
        links: [
      ...
      {
        title: 'הגדרת פריסת נייר עם חלונות תצוגה',
        intro: 'פריסות הן גיליונות מרחב-נייר — הרכיבו תצוגה אחת או יותר בקנה מידה של המודל שלכם על עמוד, מוכן להדפסה.',
        steps: [
          'לחצו על <strong>לשונית פריסה</strong> בתחתית המסך כדי לעבור ממרחב המודל למרחב הנייר. כפתור <strong>+</strong> בשורת הלשוניות מוסיף פריסה חדשה.',
          '<strong>לחצו לחיצה ימנית על לשונית פריסה</strong> כדי לשנות את שמה או למחוק אותה, או כדי לפתוח את <strong>Page Manager</strong> — שם תגדירו את פורמט הנייר (A4, A3, Letter...), הכיוון וקנה המידה של יחידות שרטוט למ"מ.',
          'הקלידו <code>viewport-rectangle</code> ולחצו על שתי פינות מנוגדות כדי למקם חלון תצוגה — חלון המציג את המודל שלכם על הנייר.',
          'לחצו על חלון תצוגה כדי לבחור בו: גררו את הקצוות או הפינות שלו לשינוי גודל, גררו את נקודת האחיזה המרכזית כדי להזיז אותו, ובחרו קנה מידה מדויק (למשל <code>1:50</code>) מבורר קנה המידה בשורת הבקרה.',
          'גללו בתוך חלון תצוגה כדי לזום את תצוגת המודל, גררו עם הלחצן האמצעי כדי להזיז אותה. כשזה נראה נכון, <strong>לחצו לחיצה ימנית על חלון התצוגה</strong> ובחרו <strong>Lock</strong> כדי להגן עליו משינויים בשוגג.',
          'צריכים את אותה תצוגה פעמיים? <code>viewport-copy</code> משכפל חלון תצוגה עם קנה המידה ותצוגת המודל שלו שמורים.',
        ],
        links: [
          { label: 'מנהל עמוד', slug: 'commands/page-manager' },
          { label: 'מלבן חלון תצוגה', slug: 'commands/viewport-rectangle' },
          { label: 'העתקת חלון תצוגה', slug: 'commands/viewport-copy' },
        ],
      },
      {
        title: 'הדפסה או שמירה כ-PDF / PNG',
        intro: 'מנהל ההדפסה מעבד את השרטוט שלכם לתמונה מוכנה לנייר, עם תצוגה מקדימה חיה.',
        steps: [
          'הקלידו <code>print</code> או לחצו על כפתור Print בפאנל File.',
          'בחרו את פורמט הפלט: PDF למסמכים, PNG/JPEG/WebP לתמונות.',
          'באופן אופציונלי חתכו לאזור מהשרטוט והפעילו פלט שחור-לבן להדפסות קווים נקיות.',
      ...
      {
        title: 'גיבוי וניהול שרטוטים שמורים',
        intro: 'השרטוטים חיים באחסון הדפדפן שלכם — פרטי, אופליין, ושלכם לנהל.',
        steps: [
          'פתחו את פאנל <strong>Files</strong> כדי לראות כל שרטוט שנשמר בדפדפן זה; שחזרו או מחקו כל אחד מהם.',
          'לפני ניקוי נתוני דפדפן או מעבר בין מכשירים, ייצאו כל דבר חשוב — <code>.json</code> שומר על נאמנות מלאה, <code>.dxf</code> ממקסם תאימות.',
      ...
      {
        title: 'הוספת גופן מותאם אישית',
        intro: 'העלו קובץ .ttf משלכם והשתמשו בו בתוויות Text ו-Multileader — הגופן נשאר שמור בדפדפן שלכם לשרטוטים עתידיים.',
        steps: [
          'הקלידו <code>FontManager</code> במסוף כדי לפתוח את הדיאלוג ישירות — או, אם עורך הטקסט כבר פתוח אצלכם, לחצו על כפתור <strong>Font Manager</strong> בסרגל הכלים שלו.',
          'לחצו <strong>Add Font</strong> בכותרת התחתונה של הדיאלוג ובחרו קובץ <code>.ttf</code> — רק גופני TrueType נתמכים.',
          'שם הקובץ הופך לשם הגופן בקבוצת <strong>User</strong> (העלאת <code>MyFont.ttf</code> מוסיפה גופן בשם <code>MyFont</code>).',
          'כדי להחיל את הגופן, הציבו תווית טקסט חדשה (פקודת <code>text</code>) או לחצו לחיצה כפולה על Text או Multileader קיימים כדי לפתוח את עורך הטקסט.',
          'בחרו את הגופן החדש מהרשימה כדי להחיל אותו — עם בחירת טקסט זה משפיע רק על התווים האלה, ללא בחירה זה קובע את כל התווית.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'עורך טקסט', slug: 'interface/text-editor' },
          { label: 'פקודת Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'הצמדה ויישור מדויקים',
        intro: 'שלבו בין Grid & Snap, Vector Pins ופקודת Align כדי למקם גיאומטריה חדשה בצורה מדויקת — בלי להקליד קואורדינטות ידנית.',
        steps: [
          'הפעילו <strong>Grid</strong> ו-<strong>Snap</strong> בסרגל הבקרה כדי לנעול את הסמן לרשת סדירה — הרווח מתאים את עצמו בזמן הזום, והצמדות לגיאומטריה קיימת (נקודת קצה, אמצע, חיתוך) עדיין קודמות לרשת.',
          'רחפו מעל כל נקודת הצמדה למשך חצי שנייה כדי לנעוץ אותה עם <strong>Vector Pins</strong> (פעיל כברירת מחדל) — הנעץ מקרין קווי ייחוס אופקיים ואנכיים מקווקווים, כך שתוכלו ללחוץ על נקודה שחולקת בדיוק את אותו קואורדינטת X או Y, בלי לצייר קו עזר.',
          'נעצו שתי נקודות שונות כדי להצמיד לנקודת החיתוך שלהן — זה ממקם נקודה בדיוק ב-(X של נעץ אחד, Y של השני), ועובד גם יחד עם מעקב זווית.',
          'כדי למקם מחדש צורה קיימת על נקודות ייחוס במקום לצייר גיאומטריה חדשה, בחרו אותה והריצו <code>align</code> — לחצו על נקודת מקור ואז נקודת יעד כדי להזיז אותה, או הוסיפו זוג נקודות שני כדי גם לסובב אותה (ואופציונלית לשנות את קנה המידה שלה) למקומה.',
          'לחצו <strong>Escape</strong> בכל שלב כדי לנקות נעצים או לבטל את היישור ולהתחיל מחדש.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
  ...
  ha: {
    ...
    guides: [
      ...
      {
        title: 'Ƙara font na musamman',
        intro: 'Loda fayil ɗinka na .ttf ka yi amfani da shi a alamun Text da Multileader — font yana ci gaba da ajiye a burauzarka don zane-zane na gaba.',
        steps: [
          'Rubuta <code>FontManager</code> a tashar umarni don buɗe akwatin tattaunawa kai tsaye — ko, idan naʼurar gyaran rubutu tana buɗe tuni, danna maɓallin <strong>Font Manager</strong> a kayan aikinta.',
          'Danna <strong>Add Font</strong> a ƙasan akwatin tattaunawa ka zaɓi fayil na <code>.ttf</code> — fonts na TrueType kaɗai ake goyon baya.',
          'Sunan fayil ya zama sunan font a ƙungiyar <strong>User</strong> (loda <code>MyFont.ttf</code> yana ƙara font mai suna <code>MyFont</code>).',
          'Don yin amfani da font, sanya sabon alamar rubutu (umarnin <code>text</code>) ko danna sau biyu a kan Text ko Multileader da ke akwai don buɗe naʼurar gyaran rubutu.',
          'Zaɓi sabon font daga jerin don yin amfani da shi — idan an zaɓi rubutu, yana shafar waɗannan haruffa kaɗai; ba tare da zaɓi ba, yana saita dukkan alamar.',
        ],
        links: [
          { label: 'Font Manager', slug: 'commands/font-manager' },
          { label: 'Naʼurar Gyaran Rubutu', slug: 'interface/text-editor' },
          { label: 'Umarnin Text', slug: 'commands/text' },
        ],
      },
      {
        title: 'Daidaita da tsara daidai-daidai',
        intro: 'Haɗa Grid & Snap, Vector Pins, da umarnin Align don sanya sabon geometry daidai-daidai — ba tare da buga daidaitawa da hannu ba.',
        steps: [
          'Kunna <strong>Grid</strong> da <strong>Snap</strong> a kayan aikin sarrafawa don kulle mai nuni zuwa grid na yau da kullum — nisa yana daidaitawa yayin da kake zoom, kuma snap na geometry da ke akwai (ƙarshen tabo, tsakiya, mahaɗa) suna ci gaba da fifiko fiye da grid.',
          'Riƙe mai nuni a kan kowane tabon snap na rabin daƙiƙa don ƙulla shi da <strong>Vector Pins</strong> (a kunne tun asali) — ƙullin yana zana layukan tunani a kwance da a tsaye masu ɗigo-ɗigo, don ka iya danna tabon da ke da daidaitawar X ko Y iri ɗaya, ba tare da zana layin ginawa ba.',
          'Ƙulla tabo biyu daban-daban don snap zuwa mahaɗarsu — wannan yana sanya tabo daidai a (X na ƙulli ɗaya, Y na ɗayan), kuma yana aiki tare da bin diddigin kusurwa shi ma.',
          'Don sake sanya wani sifa da ke akwai a kan tabobin tunani maimakon zana sabon geometry, zaɓe shi ka gudanar da <code>align</code> — danna tabon tushe sannan tabon nufi don motsa shi, ko ƙara biyu na tabo na biyu don kuma juya shi (kuma idan an so canza girmansa) zuwa wurinsa.',
          'Danna <strong>Escape</strong> a kowane lokaci don share ƙulle-ƙullen ko soke daidaitawar ka sake farawa.',
        ],
        links: [
          { label: 'Grid & Snap', slug: 'interface/grid-snap' },
          { label: 'Vector Pins', slug: 'interface/vector-pins' },
