...
keywords: [CAD align コマンド, エンティティ整列 CAD, 移動回転拡大縮小, 点ペア整列, kulmanlab]
group: edit
order: 6
---
...
4. **2 番目の始点（S2）をクリック**するか、**Enter** または **Space** を押して今すぐ移動のみの整列を適用します。
...
D1 を配置した後、2 番目の始点をクリックする代わりに **Enter** または **Space** を押します。選択範囲は S1→D1 のベクトルで移動します — 回転や拡大縮小はなし — S1 を基点、D1 を目的地とする [Move](../move/) と同じです。
...
| `Enter` / `Space`（S2 ステップ） | 回転をスキップ — S1 と D1 を使って移動のみの整列を適用 |
| `Y` | 拡大縮小ありで整列を適用 |
