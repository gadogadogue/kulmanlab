---
title: New File — 空の図面を開始（KulmanLab CAD）
description: New File コマンドはキャンバスをクリアして新しい空の図面を開きます。シンプルなファイル名が自動的に生成され、ブラウザーストレージに保存されます。
keywords: [新しい CAD ファイル, 新規図面, 空のキャンバス CAD, オンラインで新規図面作成, 新規 DXF, KulmanLab 新規ファイル, キャンバスのリセット, 図面のクリア]
group: file
order: 2
---

# New File（新規ファイル）

**New File** コマンドはキャンバスをクリアして新しい空の図面を開始します。一意のファイル名が自動的に生成されます。

## 新規ファイルの作成方法

File パネルの **New File** ツールバーボタン（新規ページアイコン）をクリックします。プロンプトや確認ダイアログなしにキャンバスが即座にクリアされます。

## 新規ファイルの内容

新しく作成されたファイルは以下の状態で開始します：

- キャンバス上に**エンティティなし**。
- 色が白で線種が `Continuous` の `0` という名前の**デフォルトレイヤー** 1 つ。
- **生成されたファイル名**、`kulman.dxf` — その名前がすでに使われている場合は `kulman (2).dxf`、`kulman (3).dxf`、… となります。

ファイルはブラウザーストレージに自動保存され、[File Manager](../file-manager/)に表示され、いつでも[名前を変更](../file-manager/#ファイルの名前変更)できます。

## 警告 — 未保存の作業は破棄されます

**New File** をクリックすると、現在のキャンバス上のすべてのエンティティが警告なしに破棄されます。現在の図面を保持したい場合は、先に[Export](../export/)してください。

## New File と Import の使い分け

| 状況 | 推奨操作 |
|-----|---------|
| 最初から図面を作成する | **New File** |
| 既存の DXF または JSON ファイルを開く | [Import](../import/) |
| 図面をコピーしてバリアントを作成する | 現在のファイルを[Export](../export/)してから、コピーを[Import](../import/) |

## 関連コマンド

- [Import](../import/) — 既存の DXF または JSON 図面を開く
- [Export](../export/) — 新規作成前に図面をダウンロード
- [File Manager](../file-manager/) — ブラウザーストレージから以前の図面を復元
