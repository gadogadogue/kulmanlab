...
角度プロンプトで数字を入力する前に **C** を押すと、**Copy** モードが切り替わります — これは AutoCAD の ROTATE コマンドが使用するのと同じインラインオプションのパターンです。プロンプトは現在の状態を `[Copy=true]` / `[Copy=false]` として表示し、もう一度 **C** を押すと元に戻ります。
