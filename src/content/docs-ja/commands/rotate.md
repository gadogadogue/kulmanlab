...
| `0`〜`9`, `.` | 角度値に数字を追加 |
| `-` | 負の角度（最初の文字のみ） |
| `C` | Copy モードを切り替え（数字を入力する前） |
| `Backspace` | 最後に入力した文字を削除 |
| `Enter` | 入力した角度で回転を適用 |

## コピーを回転する

角度プロンプトで数字を入力する前に **C** を押すと、**Copy** モードが切り替わります — これは [Polyline](../polyline/) の Arc オプションが使用するのと同じインラインオプションのパターンです。プロンプトは現在の状態を `[Copy=true]` / `[Copy=false]` として表示し、もう一度 **C** を押すと元に戻ります。

Copy がオンの場合、回転を適用すると元の選択範囲はその場に変更されず残り、代わりに選択した各エンティティの新しい回転済みコピーが追加されます。Copy がオフ（デフォルト）の場合、選択範囲は通常どおりその場で回転します。
...
| `C` | Copy モードを切り替え（角度フェーズ、数字を入力する前） |
| `Backspace` | 最後に入力した文字を削除 |
| `Enter` | 座標を確定または回転を適用 |
| `Escape` | キャンセルしてリセット |
