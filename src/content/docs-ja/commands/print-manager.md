...
品質を高くすると、同じ物理サイズでより大きく鮮明な画像が生成されます — 線の太さは解像度と一緒にスケールするため、DPI が上がるほど細く見えるのではなく、どの品質設定でも用紙上の線の*物理的な*太さは変わりません。唯一の例外はヘアライン（線幅 `0`）で、これは慣例的に「出力デバイスが描画できる最も細い線」と定義されています — ライブキャンバス上の挙動と同じく、どの品質レベルでも常に 1 ピクセル幅のままです。
