---
title: "KulmanLab CAD — Marejeo ya Amri"
description: "Marejeo ya amri za KulmanLab CAD — mwongozo kamili wa kila amri ya kuchora, kuhariri, kuweka alama, safu, kupima, na faili katika KulmanLab CAD."
keywords: [KulmanLab, KulmanLab CAD, amri za CAD, CAD ya kivinjari bila malipo, mhariri wa DXF mtandaoni, amri za kuchora, amri za kulmanlab]
group: overview
order: 1
---

# KulmanLab CAD — Marejeo ya Amri

Karibu kwenye marejeo ya amri za **KulmanLab CAD**. [KulmanLab CAD](https://kulmanlab.com) ni chombo cha CAD kinachotegemea kivinjari bila malipo kwa ajili ya kuchora, kuhariri, na kusafirisha faili za DXF — hakuna usakinishaji unaohitajika. Tumia upau wa pembeni kuvinjari amri zote zinazopatikana zilizopangwa kwa jopo.

## Maumbo

| Amri | Inachofanya |
|------|------------|
| [Line](./commands/line/) | Chora mstari ulio wazi kati ya pointi mbili |
| [Polyline](./commands/polyline/) | Chora njia wazi yenye sehemu nyingi |
| [Rectangle](./commands/rectangle/) | Chora mstatili uliolingana na mhimili |
| [Circle](./commands/circle/) | Chora duara kwa kituo na radi |
| [Arc](./commands/arc/) | Chora upinde kupitia pointi tatu |
| [Ellipse](./commands/ellipse/) | Chora duaradufu kwa kituo na mihimili miwili |
| [Hatch](./commands/hatch/) | Jaza eneo linalozunguka nukta iliyobofya kwa muundo |
| [Text](./commands/text/) | Weka lebo ya maandishi kwenye turubai |
| [Spline CV](./commands/spline-cv/) | Chora spline kwa kuweka vipeo vya udhibiti |
| [Spline Fit](./commands/spline-fit/) | Chora spline inayopita kupitia pointi zilizopigiwa klik |

## Hariri

| Amri | Inachofanya |
|------|------------|
| [Move](./commands/move/) | Hamisha vipengele vilivyochaguliwa hadi nafasi mpya |
| [Copy](./commands/copy/) | Nakili vipengele vilivyochaguliwa hadi nafasi mpya |
| [Rotate](./commands/rotate/) | Zungusha vipengele vilivyochaguliwa karibu na pointi ya msingi |
| [Mirror](./commands/mirror/) | Onyesha kioo vipengele vilivyochaguliwa kupitia mstari |
| [Scale](./commands/scale/) | Pima ukubwa wa vipengele vilivyochaguliwa karibu na pointi ya msingi |
| [Align](./commands/align/) | Hamisha, zungusha, na pima kwa hiari vipengele kwa kutumia jozi za pointi |
| [Delete](./commands/delete/) | Ondoa vipengele vilivyochaguliwa kutoka kwenye mchoro |
| [Trim](./commands/trim/) | Kata sehemu ya mstari katika makutano yake |
| [Extend](./commands/extend/) | Ongeza mstari hadi makutano ya mpaka wa karibu |
| [Offset](./commands/offset/) | Unda nakili sambamba ya kipengele kwa umbali fulani |
| [Fillet](./commands/fillet/) | Pinda kona kati ya mistari miwili kwa upinde unaogusa |
| [Chamfer](./commands/chamfer/) | Kata kona ya mstari wa diagonal kati ya mistari miwili au polylines |
| [Explode](./commands/explode/) | Inavunja polyline kuwa mistari na mikunjo binafsi |
| [Undo](./commands/undo/) | Tendua hatua ya mwisho |
| [Redo](./commands/redo/) | Rudia hatua ya mwisho iliyotendulewa |
| [Array Grid](./commands/array-grid/) | Rudia vipengele katika gridi ya mstatili ya safu mlalo na safu wima |

## Alama

| Amri | Inachofanya |
|------|------------|
| [Leader](./commands/leader/) | Chora maelezo ya multileader yenye ncha ya mshale na maandishi |
| [Leader+](./commands/leader-add/) | Ongeza mkono wa ziada kwenye multileader iliyopo |
| [Leader−](./commands/leader-remove/) | Ondoa mkono kutoka kwenye multileader iliyopo |
| [Dimension Linear](./commands/dim-linear/) | Ongeza kipimo cha usawa au wima |
| [Dimension Aligned](./commands/dim-aligned/) | Ongeza kipimo kilicholingana na pointi mbili |
| [Dimension Continue](./commands/dim-continue/) | Unganisha kipimo kipya kutoka kwa cha mwisho |
| [Dimension Radius](./commands/dim-radius/) | Ongeza kipimo cha radi kwa duara au upinde |
| [Dimension Diameter](./commands/dim-diameter/) | Ongeza kipimo cha kipenyo kwa duara |
| [Dimension Angular](./commands/dim-angular/) | Ongeza kipimo cha pembe kwa mistari miwili, upinde, au duara |

## Safu

| Amri | Inachofanya |
|------|------------|
| [LayerMakeCurrent](./commands/layer-make-current/) | Weka safu ya sasa ilingane na safu ya kipengele kilichopigiwa klik |
| [LayerMatch](./commands/layer-match/) | Gawia upya vipengele vilivyochaguliwa ili vilingane na safu ya kipengele chanzo |
| [LayerIsolate](./commands/layer-isolate/) | Ganda safu zote isipokuwa zile za vipengele vilivyochaguliwa |
| [LayerUnfreezeAll](./commands/layer-unfreeze-all/) | Fungua safu zote zilizogandwa kwa hatua moja |

## Mipango

| Amri | Inachofanya |
|------|------------|
| [ViewportRectangle](./commands/viewport-rectangle/) | Unda dirisha la kuangalia katika mpangilio wa karatasi kwa kuchagua pembe mbili |
| [ViewportCopy](./commands/viewport-copy/) | Nakili dirisha la kuangalia hadi nafasi mpya |
| [PageManager](./commands/page-manager/) | Hariri ukubwa wa karatasi na kiwango cha ukubwa kwa mpangilio unaotumika |

## Urambazaji

| Amri | Inachofanya |
|------|------------|
| [Pan](./commands/pan/) | Bonyeza na buruta ili kusongesha dirisha la kuangalia |
| [Zoom In](./commands/zoom-in/) | Karibishe dirisha la kuangalia |
| [Zoom Out](./commands/zoom-out/) | Rejesha nyuma dirisha la kuangalia |
| [Fit](./commands/fit/) | Ingiza vipengele vyote kwenye dirisha la kuangalia |

## Pima

| Amri | Inachofanya |
|------|------------|
| [Distance](./commands/distance/) | Pima umbali kati ya pointi mbili |
| [Angle](./commands/angle/) | Pima pembe kati ya pointi tatu |
| [Area](./commands/area/) | Pima eneo na mzunguko wa poligoni |

## Mtindo

| Amri | Inachofanya |
|------|------------|
| [Match Properties](./commands/match-properties/) | Nakili rangi, safu, na mali nyingine kutoka kipengele kimoja hadi vingine |
| [Font Manager](./commands/font-manager/) | Vinjari, chagua, na pakia fonti maalum za TTF |
| [Hatch Manager](./commands/hatch-manager/) | Vinjari maktaba ya muundo wa hatch na pakia faili za .pat |

## Faili

| Amri | Inachofanya |
|------|------------|
| [Import](./commands/import/) | Fungua faili ya mchoro ya DXF au JSON |
| [New File](./commands/new-file/) | Anza mchoro mpya tupu |
| [File Manager](./commands/file-manager/) | Vinjari, badilisha jina, au futa michoro iliyohifadhiwa kwenye kivinjari chako |
| [Print Manager](./commands/print-manager/) | Safirisha eneo la mchoro kama picha au PDF |
| [Export Manager](./commands/export-manager/) | Pakua mchoro kama DXF au JSON |
| [WipeStorage](./commands/wipestorage/) | Futa michoro yote kutoka kwenye hifadhi ya kivinjari |

## Uokoaji

Ikiwa programu inaanguka kila inapowasha (kwa mfano, baada ya kufanya kazi na kuratibu kubwa sana), unaweza kufuta data yote iliyohifadhiwa ndani kwa kuongeza `?reset` kwenye URL:

```
https://kulmanlab.com/?resetKulmanLocalStorage
```

Hii inafuta kila kitu kutoka kwenye hifadhidata ya ndani ya kivinjari na kuanza mchoro mpya tupu. Kigezo cha `?reset` kinajiondoa kutoka kwenye URL kiotomatiki. Tumia hii kama suluhisho la mwisho wakati [WipeStorage](./commands/wipestorage/) haiwezekani kwa sababu programu haifunguki kabisa.

## Jinsi amri zinavyofanya kazi

Kila amri inafuata muundo huo huo:

1. **Amilisha** — bonyeza kitufe cha upau wa zana au andika jina la amri kwenye terminal chini ya skrini.
2. **Fuata kidokezo** — terminal inaonyesha ingizo linalotarajiwa linalofuata.
3. **Maliza au ghairi** — amri nyingi zinamaliza kiotomatiki baada ya ingizo la mwisho. Bonyeza **Escape** wakati wowote ili kughairi.

## Kuchagua vitu

Amri kadhaa za kuhariri (Move, Copy, Rotate, Mirror, Scale, Delete) zinashiriki tabia ile ile ya uchaguzi:

- **Bonyeza** kipengele ili kukichagua au kukiachilia.
- **Buruta kulia** (kushoto kwenda kulia) kwa uchaguzi mkali — vipengele vilivyo ndani ya sanduku kikamilifu tu ndiyo vinavyochaguliwa.
- **Buruta kushoto** (kulia kwenda kushoto) kwa uchaguzi wa makutano — kipengele chochote kinachokutana na sanduku kinachaguliwa.
- Bonyeza **Enter** au **Space** kuthibitisha uchaguzi na kuendelea na hatua inayofuata.
