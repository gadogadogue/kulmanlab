...
**DXF 中不包含：** 标注图元、多重引线和 Hatch。标注和多重引线使用 KulmanLab 特定的数据结构，无法在标准 DXF 中准确表示；Hatch 目前虽然可以从 DXF 导入，但尚完全无法导出到 DXF。如果您的图形包含以上任何一种，请使用 JSON 或[Print](../print/)来捕获它们。
