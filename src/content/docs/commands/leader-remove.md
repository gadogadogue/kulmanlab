---
title: "LeaderRemove Command — Remove an Arrowhead Arm from a Multileader"
description: "The LeaderRemove command removes one arrowhead arm from a multileader that has two or more. Hover near the arm to remove — the closest arm is highlighted. The dogleg, text, and remaining arms are preserved."
...
The `LeaderRemove` command removes one arrowhead arm from an existing multileader. The text label, dogleg, and all remaining arms are preserved — only the selected arm is deleted. A multileader with only one arm cannot have its arm removed.

## Removing an arm

1. Type `LeaderRemove` in the terminal.
...
| [LeaderAdd](../leader-add/) | Add an arm to an existing multileader |
